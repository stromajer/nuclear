...
{
  struct sockaddr_in sa;
  char nlsite[81];              /* XXX: Use NI_MAXHOST */
  NL_OBJECT nl, nl2;
  RM_OBJECT rm;
  int sock;
  ...
  strcpy(nlsite, inet_ntoa(sa.sin_addr));
  write_syslog(NETLOG, 1,
...
exec_netcom(NL_OBJECT nl, char *inpstr)
{
  ...
  while (*inpstr) {
    ...
    *w3 = '\0';
    ...
    c[1] = '\0';
    /* Get command number */
    for (netcom_num = 0; netcom[netcom_num]; ++netcom_num) {
      if (!strcmp(netcom[netcom_num], w1)) {
        break;
      }
    }
    /* not found */
    if (!netcom[netcom_num]) {
      netcom_num = -1;
    }
...
nl_granted(NL_OBJECT nl, char *name)
{
  ...
  /*
     ...
     lagged then changed his mind and went elsewhere. Do not worry about it
   */
  if (u->remote_com != GO) {
    write_syslog(NETLOG, 1,
...
nl_denied(NL_OBJECT nl, char *name, char *inpstr)
{
  ...
  if (!strcmp(name, "CONNECT")) {
    ...
    sprintf(text, "~OLSYSTEM:~RS Connection to %s failed, %s.\n", nl->service,
            neterr[errnum - 1]);
    vwrite_level(command_table[CONN].level, 1, NORECORD, NULL,
                 "~OLSYSTEM:~RS Connection to %s failed, %s.\n", nl->service,
                 neterr[errnum - 1]);
 ...
 * tries to .go back to his home site or user is booted off. Home site does not
