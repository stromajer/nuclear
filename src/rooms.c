 ...
 */
int
has_room_key(char *visitor, RM_OBJECT rm)
{
  char rmname[ROOM_NAME_LEN + 1];
  UR_OBJECT u;
  FU_OBJECT fu;

  /* get room owner from room name */
  midcpy(rm->name, rmname, 1, strlen(rm->name) - 2);
  *rmname = toupper(*rmname);
  /* get owner */
  u = retrieve_user(NULL, rmname);
  if (!u) {
    return 0;
  }
  /* check flags */
...
personal_room_lock(UR_OBJECT user)
{
  ...
  if (!rm) {
    write_user(user,
               "Sorry, but you cannot use the room locking feature at this time.\n");
    return;
  }
  if (user->room != rm) {
    write_user(user,
...
personal_room_decorate(UR_OBJECT user, char *inpstr)
{
  ...
  if (inpstr) {
    ...
    if (!rm) {
      write_user(user,
                 "Sorry, but you cannot use the room decorating feature at this time.\n");
      return;
    }
    if (user->room != rm) {
      write_user(user,
...
personal_room_bgone(UR_OBJECT user)
{
  ...
  if (!rm) {
    write_user(user,
               "Sorry, but you cannot use the personal room bgone feature at this time.\n");
    return;
  }
  if (user->room != rm) {
    write_user(user,
               "You have to be in your personal room bounce people from it.\n");
    return;
  }
  /* get room to bounce people to */
  rmto = get_room(default_warp);
  if (!rmto) {
    write_user(user,
  ...
  if (!strcmp(word[1], "all")) {
    for (u = user_first; u; u = u->next) {
      if (u == user || u->room != rm || u->level == GOD) {
        continue;
      }
 ...
 */
int
personal_room_store(char *name, int store, RM_OBJECT rm)
{
  FILE *fp;
  char filename[80], line[ARR_SIZE + 1], c;
  int i;
  ...
  if (!store) {
    ...
    *name = toupper(*name);
    ...
    if (!fp) {
      ...
      strcpy(rm->desc, default_personal_room_desc);
      strcpy(rm->topic, "Welcome to my room!");
      sprintf(rm->show_name, "(%s)", name);
      strtolower(rm->show_name);
      return 0;
    }
    sprintf(rm->name, "(%s)", name);
    strtolower(rm->name);
...
personal_room_admin(UR_OBJECT user)
{
  ...
  if (!strcmp(word[1], "-m")) {
    ...
    vwrite_user(user,
                "| room structure : ~OL%8d~RS bytes  total memory : ~OL%8d~RS bytes  (%02.3f Mb) |\n",
                rsize, trsize, trsize / 1048576.0);
    vwrite_user(user,
                "|    total rooms : ~OL%8d~RS              status : ~OL%2d~RS locked, ~OL%2d~RS unlocked     |\n",
...
personal_room_rename(UR_OBJECT user, char *inpstr)
{
  ...
  if (!rm) {
    write_user(user,
               "Sorry, but you cannot use the room renaming feature at this time.\n");
    return;
  }
  if (user->room != rm) {
  ...
  length = strlen(inpstr) - teslen(inpstr, 0);
  if (!length) {
    write_user(user, "You must enter a room name.\n");
    return;
  }
  strcpy(rm->show_name, inpstr);
...
go(UR_OBJECT user)
{
  ...
  if (nl && !strncmp(nl->service, word[1], strlen(word[1]))) {
    ...
    /*
       Cannot let remote user jump to yet another remote site because this will
       ...
       but it means way too much rehacking of the code and I do not have the
...
move_user(UR_OBJECT user, RM_OBJECT rm, int teleport)
{
  ...
  if (user->vis) {
    ...
    case 0:
      ...
      break;
    case 1:
      vwrite_room(rm, "~FC~OL%s appears in an explosion of blue magic!\n",
                  user->bw_recap);
      vwrite_room_except(old_room, user,
                         "~FC~OL%s chants a spell and vanishes into a magical blue vortex!\n",
                         user->bw_recap);
      break;
    case 2:
      write_user(user,
                 "\n~FC~OLA giant hand grabs you and pulls you into a magical blue vortex!\n");
      vwrite_room(rm, "~FC~OL%s falls out of a magical blue vortex!\n",
                  user->bw_recap);
#ifdef NETLINKS
      if (!old_room) {
        sprintf(text, "REL %s\n", user->name);
...
#endif
        vwrite_room_except(old_room, user,
                           "~FC~OLA giant hand grabs %s who is pulled into a magical blue vortex!\n",
                           user->bw_recap);
      break;
    }
...
move(UR_OBJECT user)
{
  ...
  if (word_count < 2) {
    write_user(user, "Pouzitie: move <user> [<room>]\n");
    return;
  }
  u = get_user_name(user, word[1]);
  if (!u) {
  ...
  if (u->login) {
    write_user(user, "Tento user tu prave nie je.\n");
    return;
  }
  if (word_count < 3) {
    rm = user->room;
  ...
  if (user == u) {
    write_user(user,
               "Sam seba ??? EE... sorac toto nepojde.\n");
    return;
  }
  if (u->level >= user->level) {
    write_user(user,
               "Nemozes presunut user-a s tymto levelom.\n");
    return;
  }
  if (rm == u->room) {
    vwrite_user(user, "%s~RS uz predsa je v miestnosti %s.\n", u->recap, rm->name);
    return;
  };
  if (!has_room_access(user, rm)) {
    vwrite_user(user,
                "Ruumka %s je prave privatna, %s~RS tam nemoze byt preneseny(a).\n",
                rm->name, u->recap);
    return;
  }
  write_user(user, "~FC~OLMrmles si nezmyselne slova...\n");
  name = user->vis ? user->bw_recap : invisname;
  write_monitor(user, user->room, 0);
  vwrite_room_except(user->room, user,
                     "~FC~OL%s zamrmlal par nezmyselnych slov...\n", name);
  move_user(u, rm, 2);
  prompt(u);
}
 ...
 */
void
set_room_access(UR_OBJECT user)
{
  UR_OBJECT u;
  RM_OBJECT rm;
  const char *name;
  ...
  if (rm == user->room) {
    vwrite_room_except(rm, user, "%s~RS has set the room to %s.\n", name,
                       is_private_room(rm) ? "~FRPRIVATE" : "~FGPUBLIC");
  } else {
    vwrite_room(rm, "This room has been set to %s.\n",
                is_private_room(rm) ? "~FRPRIVATE" : "~FGPUBLIC");
  }
  vwrite_user(user, "Room set to %s.\n",
              is_private_room(rm) ? "~FRPRIVATE" : "~FGPUBLIC");
}
...
uninvite(UR_OBJECT user)
{
  ...
  if (u == user) {
    write_user(user, "Nemozes zrusit pozvanie sam pre seba.\n");
    return;
  }
  if (!u->invite_room) {
    vwrite_user(user, "%s~RS nebol(a) nikde pozvany(a).\n", u->recap);
    return;
  }
  if (strcmp(u->invite_by, user->name)) {
    vwrite_user(user, "%s~RS je user, ktoreho si nikam nepozval!\n",
                u->recap);
    return;
  }
  ...
  if (word_count < 2) {
    ...
    if (show_topics) {
      write_user(user,
                 "\n+----------------------------------------------------------------------------+\n");
      write_user(user,
                 "~FC~OLpl  u/m~RS  | ~OL~FCnazov~RS                 - ~FC~OLtopic\n");
      write_user(user,
                 "+----------------------------------------------------------------------------+\n");
    } else {
      write_user(user,
                 "\n+----------------------------------------------------------------------------+\n");
      write_user(user,
                 "~FC~OLNazov miestnosti    ~RS|~FC~OL Pristup  Useri  Mesgs  Inlink  LStat  Service\n");
      write_user(user,
                 "+----------------------------------------------------------------------------+\n");
    }
    rm_cnt = 0;
    for (rm = user->wrap_room; rm; rm = rm->next) {
      ...
      if (rm_cnt == user->pager - 4) {
        ...
        write_user(user, "~BB~FG-=[*]=- MACKNI <ENTER>, E PRE UKONCENIE:~RS ");
        return;
      }
      cnt = 0;
      for (u = user_first; u; u = u->next)
...
join(UR_OBJECT user)
{
  ...
  if (word_count < 2) {
    write_user(user, "Pouzitie: join <user>\n");
    return;
  }
  if (user->lroom == 2) {
    write_user(user, "Bol si znehybneny a nemozes sa pohnut.\n");
    return;
  }
  u = get_user_name(user, word[1]);
  if (!u) {
  ...
  if (user == u) {
    write_user(user,
               "Pridat sa sam k sebe? Mno neviem....\n");
    return;
  }
  rm = u->room;
#ifdef NETLINKS
...
#endif
  if (rm == user->room) {
    vwrite_user(user, "%s je s tebou v miestnosti %s.\n", u->recap,
                rm->show_name);
    return;
  }
  move_user(user, rm, 0);
  vwrite_user(user,
              "~FC~OLPrisel si k user-ovi ~RS %s~RS ~FC~OLdo miestnosti %s.~RS\n",
...
set_topic(UR_OBJECT user, char *inpstr)
{
  ...
  if (word_count < 2) {
    if (!*rm->topic) {
      write_user(user, "Topic zatial nenastaveny.\n");
      return;
    }
    vwrite_user(user, "Aktualny topic: %s\n", rm->topic);
    return;
  }
  if (strlen(inpstr) > TOPIC_LEN) {
    write_user(user, "Topic prilis dlhy.\n");
    return;
  }
  ...
  vwrite_user(user, "Topic nastaveny na: %s\n", inpstr);
  name = user->vis ? user->recap : invisname;
  vwrite_room_except(rm, user, "%s~RS nastavil topic na: %s\n", name,
                     inpstr);
...
reload_room_description(UR_OBJECT user)
{
  ...
  *pat = '\0';
  ...
  fclose(fp);
  vwrite_user(user,
              "You have now reloaded the desctiption for the room \"%s\".\n",
              rm->name);
...
reset_access(RM_OBJECT rm)
{
  ...
  if (cnt < amsys->min_private_users) {
    ...
    clear_revbuff(rm);
    rm->access &= ~PRIVATE;
    write_room(rm, "Pristup do miestnosti nastaveny na ~FGVEREJNY.\n");
  }
}
...
has_room_access(UR_OBJECT user, RM_OBJECT rm)
{
  ...
  if (priv_room[i].name) {
    return user->level >= priv_room[i].level;
  }
  if (!is_private_room(rm)) {
    return 1;
  }
...
check_start_room(UR_OBJECT user)
{
  ...
  rm = user->level == JAILED ? get_room(default_jail)
    : !user->lroom ? room_first : get_room(user->logout_room);
