...
{
  if (amsys->password_echo || user->show_pass) {
    return;
  }
 ...
 */
char *
replace_string(char *inpstr, const char *old, const char *new)
{
  size_t old_len, new_len;
  char *x, *y;

  x = istrstr(inpstr, old);
  if (!x) {
    return x;
  }
  old_len = strlen(old);
 ...
 */
void
midcpy(const char *strf, char *strt, size_t fr, size_t to)
{
  size_t f;

  /* FIXME: Check if fr > strlen(strf) */
  for (f = fr; f <= to; ++f) {
    if (!strf[f]) {
      break;
    }
...
long_date(int which)
{
  static const char *const full_month_name[] =
    { "Januar", "Februar", "Marec", "April", "Maj", "Jun", "Jul",
"August", "September", "Oktober", "November", "December" };
  static const char *const full_day_name[] =
...
smiley_type(const char *str)
{
  ...
  static const char *const frown_emotions[] =
    { ");", ")=", "):", "=(", ":(", NULL };
  static const char *const smile_emotions[] =
    { "(=", "(:", "=)", ":)", NULL };
