...
/***************************************************************************/
/* KvoCo: prelozil som tento subor skoro cely, dobre by ale bolo spravit   */
/* sklonovanie pre inphr !                                                 */
/***************************************************************************/
/*
...
create_system(void)
{
  ...
  amsys = malloc(sizeof *amsys);
  if (!amsys) {
    fprintf(stderr,
            "Angel Talker: Nie je mozne vytvorit objekt v create_system().\n");
    boot_exit(21);
    return;
  }
...
create_user(void)
{
  ...
  if (!user) {
    write_syslog(SYSLOG | ERRLOG, 0,
                 "ERROR: zlyhalo zadanie pamate v create_user().\n");
    return user;
  }
...
reset_user(UR_OBJECT user)
{
  ...
  *user->recap = '\0';
  *user->bw_recap = '\0';
  strcpy(user->desc, "je novacik bez descu.");
  strcpy(user->in_phrase, "prisiel/prisla");
  strcpy(user->out_phrase, "odchadza");
  *user->afk_mesg = '\0';
  *user->pass = '\0';
  *user->last_site = '\0';
  *user->page_file = '\0';
...
create_room(void)
{
  ...
  if (!room) {
    fprintf(stderr, "Angel Talker: zlyhalo priradenie pamate v create_room().\n");
    boot_exit(1);
    return room;
  }
...
add_command(int cmd_id)
{
  ...
  if (!cmd) {
    write_syslog(SYSLOG | ERRLOG, 0,
                 "ERROR: zlyhalo priradenie pamate v add_command().\n");
    return 0;
  }
 ...
 */
int
add_user_node(char *name, int level)
{
  UD_OBJECT new;
  ...
  if (!new) {
    write_syslog(SYSLOG | ERRLOG, 0,
                 "ERROR: zlyhalo priradenie pamate v add_user_node().\n");
    return 0;
  }
  memset(new, 0, (sizeof *new));
  if (!first_user_entry) {
    first_user_entry = new;
    new->prev = NULL;
  } else {
    last_user_entry->next = new;
    new->prev = last_user_entry;
  }
  new->next = NULL;
  last_user_entry = new;
  ++amsys->user_count;
 ...
 */
int
user_list_level(char *name, int lvl)
{
  UD_OBJECT entry;
...
save_flagged_users(UR_OBJECT user)
{
  ...
  if (!fp) {
    write_syslog(SYSLOG | ERRLOG, 1,
                 "ERROR: nemozem ulozit oznaceny user list %s\n", user->name);
    return 0;
  }
...
get_user_name(UR_OBJECT user, const char *name)
{
  ...
  *text = '\0';
  ...
  if (found > 1) {
    if (user) {
      strcat(text, found % 8 ? "\n\n" : "\n");
      vwrite_user(user, "~FR~OLName is not unique. \"~FT%s~RS~FR~OL\" also matches:\n", name);
...
retrieve_user(UR_OBJECT user, const char *name)
{
  ...
  *text = '\0';
  ...
  u = create_user();
  if (!u) {
    sprintf(text, "%s: Nie je mozne vytvorit provizorny subor uzivatela.\n", syserror);
    if (user) {
      write_user(user, text);
    }
                           ...
                           const char *buffer, unsigned flags)
{
  ...
  rb->name = malloc(1 + strlen(name));
  if (!rb->name) {
    free(rb);
    return 0;
  }
  rb->buffer = malloc(1 + strlen(buffer));
  if (!rb->buffer) {
    free(rb->name);
    free(rb);
    return 0;
