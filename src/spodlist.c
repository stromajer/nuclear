...
Prelozil som to tu trochu a pridal som design od nas tj. .+'`'+...+'`'+.
problem by mohol byt s riadkom na konci kde sa pise ze co spodlist zobrazuje
Zobrazujem od 1. po 15. (celkovo z 15 userov)
tak to ktosi pozrite
...
show_spodlist(UR_OBJECT user)
{
  ...
  write_user(user,
             "~FC'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+\n");
  write_user(user,
             "~FC+ ~FG~OLRebricek najvacsich zavislakov AngelTalkeru~RS                                ~FC+\n");
  write_user(user,
             "~FC'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+'`'+...+\n");
  ...
  for (pos = 1; pos <= end_pos && sp; ++pos, sp = sp->next) {
    if (pos >= start_pos) {
      vwrite_user(user, "~FC+ ~OL~FW%s%4d. %-20s   %45s~RS ~FC+\n",
  ...
  write_user(user,
             align_string(0, 78, 1, " ",
                          "  ~OL~FGZobrazujem pozicie od %d. do %d. miesta (~FGcelkovo z %d uzivatelov~OL~FG) ",
