...
retrieve_rooms(void)
{
  ...
  for (fscanf(f, "%s", rmname); !feof(f); fscanf(f, "%s", rmname)) {
...
retrieve_users(void)
{
  char name[USER_NAME_LEN], rmname[ROOM_NAME_LEN],
    filename[USER_NAME_LEN + 15], line[ARR_SIZE * 3 + 1];
  ...
  for (fscanf(f, "%s %s", name, rmname);
       !feof(f); fscanf(f, "%s %s", name, rmname)) {
    ...
    pf = fopen(filename, "r");
    if (pf) {
      u->malloc_start = malloc(MAX_LINES * 81);
      if (u->malloc_start) {
        memset(u->malloc_start, 0, MAX_LINES * 81);
        u->malloc_end = u->malloc_start;
        *u->malloc_start = '\0';
        for (c = getc(pf); !feof(pf); c = getc(pf)) {
          *u->malloc_end++ = c;
        }
      }
      fclose(pf);
    ...
    if (pf) {
      ...
      fscanf(pf, "%d\n", &u->pm_currcount);
      /* get data */
      for (fgets(line, ARR_SIZE, pf); !feof(pf); fgets(line, ARR_SIZE, pf)) {
        add_pm(u, line);
      }
      fclose(pf);
      /* position */
    ...
    if (pf) {
      /* get data */
      for (fgets(line, ARR_SIZE * 3, pf);
           !feof(pf); fgets(line, ARR_SIZE * 3, pf)) {
        if (wordfind(line)) {
          strcpy(text, remove_first(remove_first(line)));
