...
count_suggestions(void)
{
  ...
  valid = 1;
  for (fgets(line, 82, fp); !feof(fp); fgets(line, 82, fp)) {
    if (*line == '\n') {
      valid = 1;
    }
    sscanf(line, "%s", id);
...
check_messages(UR_OBJECT user, int chforce)
{
  ...
  for (rm = room_first; rm; rm = rm->next) {
    ...
    for (fgets(line, 82, infp); !feof(infp); fgets(line, 82, infp)) {
      if (*line == '\n') {
        valid = 1;
      }
...
{
  char c, *service, filename[80], header[ARR_SIZE];
  const char *cc;
  struct stat stbuf;
  FILE *infp, *outfp;
#ifdef NETLINKS
  ...
  if (infp) {
    ...
    /* Copy rest of file */
    for (c = getc(infp); !feof(infp); c = getc(infp)) {
      putc(c, outfp);
    }
    fclose(infp);
  }
 ...
 */
void
send_external_mail(NL_OBJECT nl, UR_OBJECT user, char *to, char *ptr)
{
  char filename[80];
  FILE *fp;
  ...
  if (!fp) {
    ...
    write_syslog(SYSLOG | ERRLOG, 0,
                 "ERROR: Cannot open file %s to append in send_external_mail().\n",
                 filename);
    return;
  }
  putc('\n', fp);
...
read_specific_mail(UR_OBJECT user)
{
  ...
  write_user(user, "\n");
  for (; !feof(fp); fgets(line, ARR_SIZE - 1, fp)) {
    if (*line == '\n') {
      break;
    }
...
mail_from(UR_OBJECT user)
{
  ...
  for (fgets(line, ARR_SIZE - 1, fp); !feof(fp);
    ...
    sscanf(line, "%s", w1);
    if (valid && (!strcmp(w1, "~OLFrom:") || !strcmp(w1, "From:"))) {
      vwrite_user(user, "~FC%2d)~RS %s", ++cnt, remove_first(line));
      valid = 0;
    }
  }
  fclose(fp);
...
send_broadcast_mail(UR_OBJECT user, char *ptr, enum lvl_value lvl, int all)
{
  ...
  for (entry = first_user_entry; entry; entry = entry->next) {
    ...
    if (infp) {
      ...
      fscanf(infp, "%d %d\r", &tmp1, &tmp2);
      /* Copy rest of file */
      for (d = getc(infp); !feof(infp); d = getc(infp)) {
        putc(d, outfp);
      }
      fclose(infp);
    }
...
forward_specific_mail(UR_OBJECT user)
{
  ...
  for (; !feof(fpi); fgets(line, ARR_SIZE - 1, fpi)) {
    if (*line == '\n') {
      break;
    }
    fprintf(fpo, "%s", colour_com_strip(line));
...
{
  char filename[80], rmname[USER_NAME_LEN];
  const char *name;
  RM_OBJECT rm;
  int ret;
...
{
  char w1[ARR_SIZE], w2[ARR_SIZE], line[ARR_SIZE], filename[80], id[ARR_SIZE];
  FILE *fp;
  ...
  *w2 = '\0';
  ...
  for (fgets(line, ARR_SIZE - 1, fp); !feof(fp);
    ...
    sscanf(line, "%s %d %s %s", id, &pt, w1, w2);
    if (valid && !strcmp(id, "PT:")) {
      valid = 0;
      if (msg_number == cnt++) {
        break;
  ...
  for (fgets(line, ARR_SIZE - 1, fp); !feof(fp);
    ...
    if (!strcmp(id, "PT:")) {
      vwrite_user(user, "~FC%2d)~RS %s", ++cnt,
                  remove_first(remove_first(remove_first(line))));
    }
  }
  fclose(fp);
...
editor(UR_OBJECT user, char *inpstr)
{
  ...
  if (!user->malloc_start) {
    user->malloc_start = malloc(MAX_LINES * 81);
    if (!user->malloc_start) {
