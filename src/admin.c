...
/*
 * Shutdown the talker
 * reboot: 0=shutdown, 1=reboot, 2=seamless reboot
 */
void
talker_shutdown(UR_OBJECT user, const char *str, int reboot)
{
 ...
 */
int
site_banned(char *sbanned, int new)
{
  ...
  for (fscanf(fp, "%s", line); !feof(fp); fscanf(fp, "%s", line)) {
...
ban_site(UR_OBJECT user)
{
  ...
  fp = fopen(filename, "r");
  if (fp) {
    for (fscanf(fp, "%s", bsite); !feof(fp); fscanf(fp, "%s", bsite)) {
...
ban_user(UR_OBJECT user)
{
  ...
  *word[2] = toupper(*word[2]);
  ...
  fp = fopen(filename, "r");
  if (fp) {
    for (fscanf(fp, "%s", name); !feof(fp); fscanf(fp, "%s", name)) {
...
unban_site(UR_OBJECT user)
{
  ...
  found = cnt = 0;
  for (fscanf(infp, "%s", ubsite); !feof(infp); fscanf(infp, "%s", ubsite)) {
...
unban_user(UR_OBJECT user)
{
  ...
  *word[2] = toupper(*word[2]);
  found = cnt = 0;
  for (fscanf(infp, "%s", name); !feof(infp); fscanf(infp, "%s", name)) {
...
check_autopromote(UR_OBJECT user, int attrib)
{
  ...
  case 3:
    ...
    ++amsys->level_count[user->level];
    vwrite_user(user,
                "\n\07~OL~FY*** Bol si automaticky povyseny na level %s ***\n\n",
                user_level[user->level].name);
    sprintf(text, "Bol automaticky povyseny na level %s.\n",
            user_level[user->level].name);
    add_history(user->name, 1, text);
    write_syslog(SYSLOG, 1, "%s bol automaticky povyseny na level %s.\n", user->name,
                 user_level[user->level].name);
    vwrite_level(WIZ, 1, NORECORD, NULL,
                 "~OL[ AUTO-PROMOTE ]~RS %s na level %s.\n", user->name,
                 user_level[user->level].name);
    return;
  }
}
...
temporary_promote(UR_OBJECT user)
{
  ...
  if (word_count < 2) {
    write_user(user, "Pouzitie: tpromote <user> [<level>]\n");
    return;
  }
  u = get_user_name(user, word[1]);
  if (!u) {
...
promote(UR_OBJECT user)
{
  ...
  lvl = -1;
  if (word_count < 2) {
    write_user(user, "Pouzitie: promote <user> [<level>]\n");
    return;
  }
  if (word_count > 3) {
    write_user(user, "Pouzitie: promote <user> [<level>]\n");
  ...
  if (word_count == 3) {
    ...
    lvl = get_level(word[2]);
    if (lvl == NUM_LEVELS) {
      vwrite_user(user, "Musis vybrat level medzi %s a %s.\n",
                  user_level[USER].name, user_level[ARCH].name);
      return;
    }
  }
  ...
  if (word_count == 3 && lvl <= u->level) {
    write_user(user,
               "Nemozes povysit user-a na level nizsi, alebo rovnaky, aky prave je.\n");
    done_retrieve(u);
    return;
  }
  if ((word_count == 3 && lvl >= user->level) || (u->level >= user->level)) {
    write_user(user,
               "Nemozes povysit na level tvoj, alebo vyssi ako tvoj.\n");
    done_retrieve(u);
    return;
  }
  /* cannot promote jailed users */
  if (u->level == JAILED) {
    vwrite_user(user, "Nemozes povysit usera s levelom %s.\n",
                user_level[JAILED].name);
    done_retrieve(u);
    return;
  }
  /* nemozes promote anjelof... */
  if (u->level == ARCH) {
    vwrite_user(user, "HEH, DALSIEHO %sA ??? to hadam nie... ;)) \n",
                user_level[GOD].name);
    done_retrieve(u);
    return;
  }
  ...
  u->level = word_count == 3 ? lvl : u->level + 1;
  u->unarrest = u->level;
  u->real_level = u->level;
  ...
  sprintf(text, "~FG~OLBol si povyseny na level: ~RS~OL%s.\n",
          user_level[u->level].name);
  if (!on) {
  ...
  vwrite_level(u->level, 1, NORECORD, u,
               "~FG~OL%s je povyseny na level: ~RS~OL%s.\n", u->bw_recap,
               user_level[u->level].name);
  write_syslog(SYSLOG, 1, "%s POVYSIL %s na level %s.\n", user->name,
               u->name, user_level[u->level].name);
  sprintf(text, "%s ho ~FGpovysil~RS na level %s.\n", user->name,
          user_level[u->level].name);
  add_history(u->name, 1, text);
  if (!on) {
    u->socket = -2;
...
demote(UR_OBJECT user)
{
  ...
  lvl = -1;
  if (word_count < 2) {
    write_user(user, "Pouzitie: demote <user> [<level>]\n");
    return;
  }
  if (word_count > 3) {
    write_user(user, "Pouzitie: demote <user> [<level>]\n");
...
change_pass(UR_OBJECT user)
{
  ...
  i = strlen(word[2]);
  if (i < PASS_MIN) {
    write_user(user, "Nove heslo je prikratke.\n");
    return;
  }
  /* Via use of crypt() */
  if (i > 8) {
    write_user(user,
               "UPOZORNENIE! Zo zadaneho hesla bude pouzitych iba prvych 8 znakov \n");
  }
  /* Change own password */
  if (word_count == 3) {
  ...
  if (user->level < GOD) {
    write_user(user,
               "Sorry, ale na toto nemas dostatocny level.\n");
    return;
  }
  *word[3] = toupper(*word[3]);
  if (!strcmp(word[3], user->name)) {
     ...
     */
    write_user(user,
               "Nemozes si zmenit heslo so zadanim <user> seba sameho.\n");
    return;
  }
  u = get_user(word[3]);
  if (u) {
    ...
    cls(user);
    vwrite_user(user, "%s~RS-ovo(-ino) heslo bolo zmenene.\n", u->recap);
    name = user->vis ? user->name : invisname;
    vwrite_user(u, "~FR~OL%s ti zmenil heslo!\n", name);
    write_syslog(SYSLOG, 1, "%s zmenil %s-ovi(-e) heslo.\n", user->name,
                 u->name);
...
user_expires(UR_OBJECT user)
{
  ...
  save_user_details(u, (retrieve_user_type % 2));
  done_retrieve(u);
}
...
site(UR_OBJECT user)
{
  ...
  if (u->type == REMOTE_TYPE) {
    vwrite_user(user, "%s is remotely connected from %s.\n", u->name,
                u->site);
    done_retrieve(u);
    return;
  }
...
minlogin(UR_OBJECT user)
{
  ...
  if (lvl == NUM_LEVELS) {
    ...
    strcpy(levstr, user_level[lvl].name);
  }
  if (lvl > user->level) {
    write_user(user,
...
system_details(UR_OBJECT user)
{
  ...
  char bstr[32], min_login[5], foo[ARR_SIZE];
  RM_OBJECT rm;
  UR_OBJECT u;
  CMD_OBJECT c;
  UD_OBJECT d;
  int days, hours, mins, secs, num_clones;
  int ssize, usize, dsize, csize, lsize, total;
  int i;
  enum lvl_value lvl;
  int tusize, tdsize, tcsize, tlsize, rsize, trsize, rmcnt, prmcnt;
#ifdef NETLINKS
  NL_OBJECT n, nl;
  int netlinks, live, inc, outg, inlinks, nsize, tnsize;
#endif
...
#endif
    vwrite_user(user, "|     talker booted : %-54s |\n", bstr);
    sprintf(text, "%d day%s, %d hour%s, %d minute%s, %d second%s", days,
            PLTEXT_S(days), hours, PLTEXT_S(hours), mins, PLTEXT_S(mins),
            secs, PLTEXT_S(secs));
    vwrite_user(user, "|            uptime : %-54s |\n", text);
    vwrite_user(user, "|    system logging : %-54s |\n",
                offon[(amsys->logging) ? 1 : 0]);
    sprintf(text, "every %d sec%s", amsys->heartbeat,
            PLTEXT_S(amsys->heartbeat));
    vwrite_user(user,
                "|         heartbeat : %-20s          swear ban : %s          |\n",
                text, minmax[amsys->ban_swearing]);
    vwrite_user(user,
                "| ignoring sigterms : %-6s                     crash action : %-10s   |\n",
                noyes[amsys->ignore_sigterm], ca[amsys->crash_action]);
    vwrite_user(user,
                "|  flood protection : %-6s                     resolving IP : %-10s   |\n",
  ...
  if (!strcasecmp("-u", word[1])) {
    ...
    for (lvl = 0; lvl < NUM_LEVELS; ++lvl) {
      sprintf(text, "users at level %s", user_level[lvl].name);
      vwrite_user(user,
                  "| %23.23s : %-5d                                            |\n",
                  text, amsys->level_count[lvl]);
    }
    write_user(user,
               "+----------------------------------------------------------------------------+\n");
    if (amsys->minlogin_level == NUM_LEVELS) {
      strcpy(min_login, "NONE");
    } else {
      strcpy(min_login, user_level[amsys->minlogin_level].name);
    }
    vwrite_user(user,
                "|      max allowed online : %-5d                    online now : %-5d      |\n",
                amsys->max_users, amsys->num_of_users);
    vwrite_user(user,
                "|           new this boot : %-5d           returning this boot : %-5d      |\n",
                amsys->logons_new, amsys->logons_old);
    vwrite_user(user,
                "|      max allowed clones : %-5d                 clones now on : %-5d      |\n",
    ...
    vwrite_user(user,
                "|     login idle time out : %-10s       user idle time out : %-10s |\n",
                text, foo);
    vwrite_user(user,
                "|           time out afks : %-10s        time out maxlevel : %-10.10s |\n",
                noyes[amsys->time_out_afks],
                user_level[amsys->time_out_maxlevel].name);
    vwrite_user(user,
                "|  current minlogin level : %-10.10s  wizport min login level : %-10.10s |\n",
                min_login, user_level[amsys->wizport_level].name);
    vwrite_user(user,
                "|   newbie prompt default : %-10s    newbie colour default : %-10s |\n",
                offon[amsys->prompt_def], offon[amsys->colour_def]);
    vwrite_user(user,
                "| newbie charecho default : %-10s      name recaps allowed : %-10s |\n",
                offon[amsys->charecho_def], noyes[amsys->allow_recaps]);
    vwrite_user(user,
                "| echoing password default: %-10s    smail auto-forwarding : %-10s |\n",
                noyes[amsys->password_echo], offon[amsys->forwarding]);
    strftime(text, ARR_SIZE * 2, "%a %Y-%m-%d %H:%M:%S",
             localtime(&amsys->auto_purge_date));
    vwrite_user(user,
                "|            autopurge on : %-4s  next autopurge : %-25s |\n",
                noyes[amsys->auto_purge_date != -1], text);
    sprintf(text, "%d day%s", USER_EXPIRES, PLTEXT_S(USER_EXPIRES));
    sprintf(foo, "%d day%s", NEWBIE_EXPIRES, PLTEXT_S(NEWBIE_EXPIRES));
    vwrite_user(user,
                "|  purge length (newbies) : %-10s     purge length (users) : %-10s |\n",
                foo, text);
    vwrite_user(user,
                "|    min login disconnect : %-10s                                       |\n",
  ...
  if (!strcasecmp("-n", word[1])) {
...
#else
    netlinks = live = inc = outg = inlinks = nsize = tnsize = 0;
    ...
    sprintf(foo, "%d sec%s", amsys->net_idle_time,
            PLTEXT_S(amsys->net_idle_time));
    vwrite_user(user,
                "|       total netlinks : %-5d                 idle time out : %-13.13s |\n",
                netlinks, foo);
    vwrite_user(user,
                "|   accepting connects : %-5d         number which are live : %-5d         |\n",
                inlinks, live);
    vwrite_user(user,
                "| incoming connections : %-5d          outgoing connections : %-5d         |\n",
                inc, outg);
    vwrite_user(user,
                "| remote user maxlevel : %-13.13s  remote user deflevel : %-13.13s |\n",
                user_level[amsys->rem_user_maxlevel].name,
                user_level[amsys->rem_user_deflevel].name);
    sprintf(text, "%d byte%s", nsize, PLTEXT_S(nsize));
    sprintf(foo, "%d byte%s", tnsize, PLTEXT_S(tnsize));
    vwrite_user(user,
                "|    netlink structure : %-13.13s          total memory : %-13.13s |\n",
                text, foo);
#endif
    write_user(user,
               "+----------------------------------------------------------------------------+\n\n");
  ...
  if (!strcasecmp("-r", word[1])) {
    ...
    write_user(user,
               "|----------------------------------------------------------------------------|\n");
    vwrite_user(user,
                "| gatecrash level : %-15.15s       priv minimum count : %-5d         |\n",
    ...
    vwrite_user(user,
                "|    message life : %-15.15s       message check time : %.2d:%.2d         |\n",
                text, amsys->mesg_check_hour, amsys->mesg_check_min);
    vwrite_user(user,
                "|     total rooms : %-15d           personal rooms : %-13d |\n",
                rmcnt, prmcnt);
    sprintf(text, "%d byte%s", rsize, PLTEXT_S(rsize));
    sprintf(foo, "%d byte%s", trsize, PLTEXT_S(trsize));
    vwrite_user(user,
                "|  room structure : %-15.15s             total memory : %-13.13s |\n",
                text, foo);
    write_user(user,
               "+----------------------------------------------------------------------------+\n\n");
    return;
  ...
  if (!strcasecmp("-m", word[1])) {
    ssize = usize = rsize = dsize = csize = lsize = 0;
    tusize = trsize = tdsize = tcsize = tlsize = 0;
    ssize = (sizeof *amsys);
    usize = (sizeof *u);
    for (u = user_first; u; u = u->next) {
      tusize += (sizeof *u);
    ...
    csize = (sizeof *c);
    for (c = first_command; c; c = c->next) {
      tcsize += (sizeof *c);
    }
    lsize = (sizeof *last_login_info);
    for (i = 0; i < LASTLOGON_NUM; ++i) {
      tlsize += (sizeof *last_login_info);
    }
    total = tusize + trsize + tdsize + tcsize + tlsize + ssize;
#ifdef NETLINKS
    nsize = tnsize = 0;
    nsize = (sizeof *n);
    for (n = nl_first; n; n = n->next)
      tnsize += (sizeof *n);
    total += tnsize;
#endif
    write_user(user,
    ...
    write_user(user,
               "|----------------------------------------------------------------------------|\n");
    vwrite_user(user,
                "|    user structure : %8d bytes    user list structure : %8d bytes |\n",
                usize, dsize);
    vwrite_user(user,
                "|                   : ~OL%8d~RS bytes                        : ~OL%8d~RS bytes |\n",
                tusize, tdsize);
    vwrite_user(user,
                "|    room structure : %8d bytes      command structure : %8d bytes |\n",
                rsize, csize);
    vwrite_user(user,
                "|                   : ~OL%8d~RS bytes                        : ~OL%8d~RS bytes |\n",
                trsize, tcsize);
    vwrite_user(user,
                "|  system structure : %8d bytes   last login structure : %8d bytes |\n",
                ssize, lsize);
    vwrite_user(user,
                "|                   : ~OL%8d~RS bytes                        : ~OL%8d~RS bytes |\n",
                ssize, tlsize);
    ...
    write_user(user,
               "+----------------------------------------------------------------------------+\n");
    vwrite_user(user,
                "| Total object memory allocated : ~OL%9d~RS bytes   (%02.3f Mb)               |\n",
 ...
 */
void
recount_users(UR_OBJECT user, int ok)
{
  char filename[80], *s;
  DIR *dirp;
  FILE *fp;
  ...
  /*
     ...
     to add and then one to remove, but it is the best way I can think of doing it right now at 4:27am!
   */
...
set_command_level(UR_OBJECT user)
{
  ...
  if (!strcmp(word[2], "NORM")) {
    ...
    if (cmd->min_lev == (enum lvl_value) command_table[cmd->id].level) {
      write_user(user, "That command is already at its normal level.\n");
      return;
    }
    cmd->min_lev = (enum lvl_value) command_table[cmd->id].level;
    write_syslog(SYSLOG, 1,
  ...
  if ((enum lvl_value) command_table[cmd->id].level > user->level) {
    write_user(user,
  ...
  write_syslog(SYSLOG, 1, "%s has set the level for cmd \"%s\" to %d (%s)\n",
               user->name, cmd->name, cmd->min_lev,
...
get_xgcoms(UR_OBJECT user)
{
  ...
  xi = 0;
  gi = 0;
  for (fscanf(fp, "%d %d", &type, &tmp); !feof(fp);
...
force(UR_OBJECT user, char *inpstr)
{
  ...
  if (word_count < 3) {
    write_user(user, "Pouzitie: force <user> <akcia>\n");
    return;
  }
  *word[1] = toupper(*word[1]);
  u = get_user_name(user, word[1]);
  ...
  if (u == user) {
    write_user(user, "Prikazat sam sebe.... SCHIIIIZA ????.\n");
    return;
  }
  if (u->level >= user->level) {
    write_user(user,
               "Toto nepojde... taqe prava nemas.....\n");
    return;
  }
  inpstr = remove_first(inpstr);
  ...
  *word[w++] = '\0';
  *word[w++] = '\0';
  vwrite_user(u, "%s ti prikazuje: \"%s\"\n", user->name, inpstr);
  vwrite_user(user, "You force %s to: \"%s\"\n", u->name, inpstr);
  if (!exec_com(u, inpstr, -1)) {
...
void
vysklonuj(UR_OBJECT user,char *inpstr)
{
  UR_OBJECT u;
  int p = 0;
  if (word_count < 3 ) {
    write_user(user, "Pouzitie: sklonuj <user> [<show,ukaz> || <pad> <tvar>]\n");
    return;
  }
  ...
  *word[1] = toupper(*word[1]);
  ...
  if (u) {
   ...
   if (strcmp(word[2],"show") == 0 || strcmp(word[2],"ukaz") == 0)
    {vwrite_user(user,"~OL~FG>>>SKLONOVANIE USERA %s<<<~RS\n NOMINATIV > %s\n GENITIV > %s\n DATIV > %s\n AKUZATIV > %s\n LOKAL > %s\n INSTRUMENTAL > %s\n",u->name, u->nomin, u->genit, u->dativ, u->akuza, u->lokal, u->instr);
     return; }
  ...
  if (strcmp(word[2],"n") == 0 || strcmp(word[2],"N") == 0)
    {strcpy(u->nomin,word[3]); p = 1;}
  if (strcmp(word[2],"g") == 0 || strcmp(word[2],"G") == 0)
    {strcpy(u->genit,word[3]); p = 1;}
  if (strcmp(word[2],"d") == 0 || strcmp(word[2],"D") == 0)
    {strcpy(u->dativ,word[3]); p = 1;}
  if (strcmp(word[2],"a") == 0 || strcmp(word[2],"A") == 0)
    {strcpy(u->akuza,word[3]); p = 1;}
  if (strcmp(word[2],"l") == 0 || strcmp(word[2],"L") == 0)
    {strcpy(u->lokal,word[3]); p = 1;}
  if (strcmp(word[2],"i") == 0 || strcmp(word[2],"I") == 0)
    {strcpy(u->instr,word[3]); p = 1;}
  save_user_details(u,0);
     if (p == 1) {vwrite_user(user,"~OL~FGOKI... moze byt...~FW %s~RS\n",inpstr);    }
     else {write_user(user, "~OL~FRZle zadany pad...check it...~RS\n");}
    }
  else {write_user(user,"~OL~FROFFLINE.. TO NERIESIM zatial....~RS Week... ;))\n"); }    
...
dump_to_file(UR_OBJECT user)
{
  ...
  FILE *fp;
  int ssize, usize, rsize, nsize, dsize, csize, lsize, total, i, j;
  ...
  n = NULL;
#endif
  ssize = usize = rsize = nsize = dsize = csize = lsize = i = j = cnt = 0;
  ...
  if (!strcmp("-m", word[1])) {
    ...
    total = tusize + trsize + tnsize + tdsize + tcsize + tlsize + ssize;
    fprintf(fp,
            "------------------------------------------------------------------------------\n");
...
shackle(UR_OBJECT user)
{
  ...
  if (!u->room) {
    vwrite_user(user,
                "%s is currently off site and cannot be shackled there.\n",
                u->name);
    return;
  }
#endif
