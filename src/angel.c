...
/**************************************************************************
                Nastavovanie talkera a cyklu hlavneho programu
                           prekladali Week & KvoCo
 **************************************************************************/
...
main(int argc, char **argv)
{
  ...
  printf
    ("\n.+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n");
  printf("AngelTalker %s server booting %s\n", AMNUTSVER, long_date(1));
  printf
    (".+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n");
  printf("System name : %s, release %s, %s\n", amsys->uts.sysname,
         amsys->uts.release, amsys->uts.version);
  printf("Running on  : %s, %s\n", amsys->uts.machine, amsys->uts.nodename);
#ifndef NETLINKS
  ...
  printf("Ochrana proti FLOODovaniu je %s.\n", offon[amsys->flood_protect]);
  if (amsys->personal_rooms) {
    if (amsys->startup_room_parse) {
      parse_user_rooms();
    } else {
      printf("Osobne roomky su aktivne, ale neboli analyzovane pri starte.\n");
    }
  } else {
    printf("Osobne roomky su zakazane\n");
  }
  printf("Kontorlujem strukturu adresarov uzivatelov\n");
  check_directories();
  printf("Nacitavam user list\n");
  process_users();
  printf("Pocitam uzivatelov\n");
  count_users();
  printf("Analyzujem strukturu prikazov\n");
  parse_commands();
  ...
  count_suggestions();
  printf("Mame %d novych nametov od uzivtelov. (.rsug).\n", amsys->suggestion_count);
  count_motds(0);
  printf("Mame %s %d login motd%s and %d post-login motd%s\n",
  ...
  if (!possibly_reboot()) {
#ifdef NETLINKS
#ifdef WIZPORT
    printf("Nacitavam sockety na portoch: %s, %s, %s\n", amsys->mport_port,
           amsys->wport_port, amsys->nlink_port);
#else
    printf("Nacitavam sockety na portoch: %s, %s\n", amsys->mport_port,
           amsys->nlink_port);
#endif
#else
#ifdef WIZPORT
    printf("Nacitavam sockety na portoch: %s, %s\n", amsys->mport_port,
           amsys->wport_port);
#else
    printf("Nacitavam sockety na portoch: %s\n", amsys->mport_port);
#endif
#endif
  ...
  if (amsys->auto_connect) {
    ...
    printf("Preskakujem fazu pripajania (zrejme netlinks, pozn. prekladu).\n");
  }
#endif
  ...
  printf
    (".+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n");
  printf("Spustene s PID %u\n", getpid());
  printf
    (".+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n\n");
  write_syslog(SYSLOG, 0,
               ".+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\nServer spusteny s  PID %u %s\n",
               getpid(), long_date(1));
  write_syslog(SYSLOG, 0,
               ".+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n\n");
 ...
 *****************************************************************************/
  ...
  for (;;) {
    ...
    if (amsys->ident_state == 1 && FD_ISSET(amsys->ident_socket, &readmask)) {
      ...
      if (!len) {
        write_level(WIZ, 1, NORECORD, "~FY<ArIdent Daemon Odpojeny>~RS\n",
                    NULL);
        amsys->ident_state = 0;
        continue;
      }
          ...
          if (!strncmp(u->site, word[1], strlen(word[1]) - 1)) {
            *u->site = '\0';
            ...
            if (site_banned(u->site, 0)) {
              write_user(u,
                         "\nLutujem, ale prihlasovanie z tvojej adresy je zakazane.\n\n");
              disconnect_user(u);
            }
          }
        }
      ...
      *inpstr = '\0';
      ...
      for (;;) {
        ...
        *user->buff = '\0';
         ...
         */
        if (!user->misc_op) {
          if (!strcmp(curstr, ".") && user->inpstr_old[0]) {
        ...
        if (user->afk) {
          if (user->afk == 2) {
            ...
            cls(user);
            write_user(user, "Odomknul si si pristup, uz nie si AFK.\n");
          } else {
            write_user(user, "Opat si aktivny, uz nie si AFK.\n");
          }
          *user->afk_mesg = '\0';
          if (has_review(user, rbfAFK)) {
            write_user(user,
                       "\nNiekto ti pisal kym si bol AFK.  Pouzi ~FCrevafk~RS pre pozretie sprav.\n\n");
          }
          if (user->vis) {
            vwrite_room_except(user->room, user,
                               "%s~RS je opat pri klavesnici.\n",
                               user->recap);
          }
          if (user->afk == 2) {
            user->afk = 0;
        ...
        com_num = -1;
        exec_com(user, curstr, user->command_mode ? -1 : SAY);
        if (!destructed) {
          if (user->room) {
            prompt(user);
          } else {
...
check_directories(void)
{
  ...
  for (i = 0; i < NUM_LEVELS; ++i) {
    for (j = i + 1; j < NUM_LEVELS; ++j) {
      if (!strcmp(user_level[i].name, user_level[j].name)) {
        fprintf(stderr, "AngelTalker: Level names are not unique.\n");
        boot_exit(14);
      }
    }
  }
  ...
  if (stat(dirname, &stbuf) == -1) {
    fprintf(stderr,
            "AngelTalker: Chyba stavu adresarov v check_directories().\n");
    boot_exit(15);
  }
  if (!S_ISDIR(stbuf.st_mode)) {
    fprintf(stderr, "AngelTalker: Struktura adresarov Je chybna.\n");
    boot_exit(16);
  }
...
accept_connection(int lsock)
{
  ...
  char buffer[40];
#endif
  char hostaddr[16];            /* XXX: Use NI_MAXHOST */
  char hostname[270];           /* XXX: Use NI_MAXHOST */
  char motdname[80];
  struct sockaddr_in sa;
  UR_OBJECT user;
  int accept_sock;
  int sa_len;                   /* socklen_t */
  ...
  *hostname = *hostaddr = '\0';
  ...
  if (site_banned(hostaddr, 0) || site_banned(hostname, 0)) {
    write_sock(accept_sock,
               "\n\rPristup z tvojej adresy je zabannovany.\n\n\r");
    close(accept_sock);
    write_syslog(SYSLOG, 1, "Pokus o pripojenie sa zo zabannovanej adresy %s (%s).\n",
                 hostname, hostaddr);
    return;
  }
  if (amsys->flood_protect
    ...
    write_sock(accept_sock,
               "\n\rPokusas sa zaFLOODovat porty talkera. Vnimame to ako\n\r");
    write_sock(accept_sock,
               "priznak toho ze chces hacknut system.  Z tohoto dovodu\n\r");
    write_sock(accept_sock, "je prihlasovanie z tvojej adresy zabannovane!.\n\n\r");
    close(accept_sock);
    write_syslog(SYSLOG, 1, "Pokus o hacknutie z adresy %s (%s).\n",
                 hostname, hostaddr);
  ...
  if (amsys->motd1_cnt) {
    ...
    sprintf(text,
            "Vitaje ta! %s!\n\n\rPrepac, ale z nejakeho dovodu nam prave chyba login obrazok.\n\r",
            TALKER_NAME);
...
#endif
  {
    if (amsys->stop_logins) {
      write_sock(accept_sock,
                 "\n\rPrepac, ale na talker prave nie je mozne sa pripojit.\n\rSkus to neskor.\n\n\r");
      close(accept_sock);
      return;
    }
    if (amsys->num_of_users + amsys->num_of_logins >= amsys->max_users) {
      write_sock(accept_sock,
                 "\n\rPrepac, ale uz nemozeme pripojit dalsich ludi.\n\rSkus to neskor.\n\n\r");
      close(accept_sock);
      return;
    }
  }
  user = create_user();
  if (!user) {
    sprintf(text, "\n\r%s: nie je mozne vytvorit pripojenie.\n\n\r", syserror);
    write_sock(accept_sock, text);
    close(accept_sock);
    return;
  }
  ...
  if (user->wizport) {
    write_user(user, ".+'`'+. Pripojenie na port Anjelov .+'`'+.\n\n");
  }
#endif
...
/******************************************************************************
                  Nacitavanie a analyzovanie konfiguracneho suboru
 
                           prelozene by KvoCo (c)
        kua toho ale bolo, tolko riadkov a aj tak sa to vykona raz za rok
 *****************************************************************************/
...
load_and_parse_config(void)
{
  ...
  printf("Analyzujem konfiguracny subor \"%s\"...\n", filename);
  fp = fopen(filename, "r");
  if (!fp) {
    perror("AngelTalker: Nemozem otvorit konfiguracny subor!");
    boot_exit(1);
  }
  /* Main reading loop */
  config_line = 0;
  for (fgets(line, 81, fp); !feof(fp); fgets(line, 81, fp)) {
    ++config_line;
    for (i = 0; i < 8; ++i) {
      *wrd[i] = '\0';
    }
    sscanf(line, "%s %s %s %s %s %s %s %s", wrd[0], wrd[1], wrd[2], wrd[3],
    ...
    if (wrd[0][strlen(wrd[0]) - 1] == ':') {
      if (!strcmp(wrd[0], "INIT:")) {
        ...
        fprintf(stderr, "AngelTalker: Neznama cast hlavickoveho suboru na riadku: %d.\n",
                config_line);
        fclose(fp);
        boot_exit(1);
      }
    ...
    case 4:
      ...
      break;
    default:
      fprintf(stderr, "AngelTalker: Sekcia hlavickoveho suboru ocakavana na riadku: %d.\n",
              config_line);
      fclose(fp);
      boot_exit(1);
    }
   ...
   */
  if (!got_init) {
    fprintf(stderr, "AngelTalker: Sekcia INIT chyba v konfiguracnom subore.\n");
    boot_exit(1);
  }
  if (got_topics && !got_rooms) {
    fprintf(stderr,
            "AngelTalker: Sekcia TOPICS musi nasledovat az za sekciou ROOMS v konfiguracnom subore.\n");
    boot_exit(1);
  }
  if (!got_rooms) {
    fprintf(stderr, "AngelTalker: Chyba sekcia ROOMS v konfiguracnom subore.\n");
    boot_exit(1);
  }
  if (!*amsys->mport_port) {
    fprintf(stderr, "AngelTalker: Main port nie je zadefinovany v konfiguracnom subore.\n");
    boot_exit(1);
  }
#ifdef WIZPORT
  if (!*amsys->wport_port) {
    fprintf(stderr, "AngelTalker: Wiz port nie je zadefinovany v konfiguracnom subore.\n");
    boot_exit(1);
  }
  if (!strcmp(amsys->mport_port, amsys->wport_port)) {
    fprintf(stderr, "AngelTalker: Cisla portov musia byt unikatne.\n");
    boot_exit(1);
  }
#endif
#ifdef NETLINKS
  if (!*amsys->nlink_port) {
    fprintf(stderr, "AngelTalker: Link port nie je zadefinovany v konfiguracnom subore.\n");
    boot_exit(1);
  }
  if (!*amsys->verification) {
    fprintf(stderr, "AngelTalker: Verification not set in config file.\n");
    boot_exit(1);
  }
...
#endif
  if (!room_first) {
    fprintf(stderr, "AngelTalker: Nie su zadefinovane ziadne miestnosti v config subore.\n");
    boot_exit(1);
  }
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    for (i = 0; i < MAX_LINKS; ++i) {
      ...
      if (!rm2) {
        fprintf(stderr, "AngelTalker: Miestnost %s nema zadefinovany nazov cesty \"%s\".\n",
                rm1->name, rm1->link_label[i]);
        boot_exit(1);
      }
      if (rm2 == rm1) {
        fprintf(stderr, "AngelTalker: Miestnost %s nemoze mat cestu sama do seba.\n",
                rm1->name);
        boot_exit(1);
      }
      rm1->link[i] = rm2;
  ...
  for (nl = nl_first; nl; nl = nl->next) {
    ...
    if (rm1) {
      fprintf(stderr, "AngelTalker: Nazov sluzby %s je taktiez menom miestnosti.\n",
              nl->service);
      boot_exit(1);
    }
  }
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    ...
    if (!nl) {
      fprintf(stderr, "AngelTalker: Nazov sluzby %s nie je definovany pre miestnost %s.\n",
  ...
  for (rm1 = room_first; rm1; rm1 = rm1->next) {
    ...
    fp = fopen(filename, "r");
    if (!fp) {
      fprintf(stderr, "AngelTalker: Nemozem najst subor s popisom pre miestnost %s.\n",
              rm1->name);
      write_syslog(SYSLOG | ERRLOG, 0,
                   "CHYBA: Nemozem najst subor s popisom pre miestnost %s.\n",
                   rm1->name);
      continue;
    }
    i = 0;
    ...
    if (!feof(fp)) {
      fprintf(stderr, "AngelTalker: Popis je priliz dlhy pre miesnost %s.\n",
              rm1->name);
      write_syslog(SYSLOG | ERRLOG, 0,
                   "CHYBA: Popis je priliz dlhy pre miesnos %s.\n", rm1->name);
    }
    rm1->desc[i++] = '\0';
    fclose(fp);
  }
...
parse_init_section(void)
{
  static const char *const options[] = {
    "mainport", "wizport", "linkport", "system_logging", "minlogin_level",
      "mesg_life",
    "wizport_level", "prompt_def", "gatecrash_level", "min_private",
      "ignore_mp_level",
    ...
    "ignore_sigterm", "auto_connect", "max_clones", "ban_swearing",
      "crash_action",
    "colour_def", "time_out_afks", "charecho_def", "time_out_maxlevel",
      "auto_purge",
    "allow_recaps", "auto_promote", "personal_rooms", "random_motds",
      "startup_room_parse",
    "resolve_ip", "flood_protect", "boot_off_min", NULL
  };
  static int in_section = 0;
  int op, val;

  if (!strcmp(wrd[0], "INIT:")) {
    /* XXX: This could be made idempotent */
    if (in_section++) {
      fprintf(stderr, "AngelTalker: Neockavana cast INIT hlavicky na riadku: %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  }
  for (op = 0; options[op]; ++op) {
    if (!strcmp(options[op], wrd[0])) {
      break;
    }
  }
  if (!options[op]) {
    fprintf(stderr, "AngelTalker: Nezname nastavenie INIT na riadku %d.\n", config_line);
    boot_exit(1);
  }
  if (!*wrd[1] || *wrd[1] == '#') {
    fprintf(stderr, "AngelTalker: Chyba potrebny parameter na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (*wrd[2] && *wrd[2] != '#') {
    fprintf(stderr,
            "AngelTalker: Neocakavane slovo nasleduje init parameter na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  val = atoi(wrd[1]);
  ...
  case 0:
    /* main port */
    if (strlen(wrd[1]) >= 32) { /* NI_MAXSERV */
      fprintf(stderr, "AngelTalker: Chybne cislo portu na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 3:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: System_logging musi byt ON alebo OFF na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 4:
    {
      ...
      if (lvl == NUM_LEVELS && strcmp(wrd[1], "NONE")) {
        fprintf(stderr,
                "AngelTalker: Zadany neznamy level pre minlogin_level na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 5:
    /* message lifetime */
    if (val < 1) {
      fprintf(stderr, "AngelTalker: Chybna sprava dlzky zivota na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    amsys->mesg_life = val;
  ...
  case 6:
    /* wizport_level */
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Zadany neznamy level pre wizport_level na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 7:
    ...
    val = onoff_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Prompt_def musi byt ON alebo OFF na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 8:
    /* gatecrash level */
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Zle zadany level pre gatecrash_level na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 9:
    if (val < 1) {
      fprintf(stderr,
              "AngelTalker: Priliz male cislo min_private_users na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 10:
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Zle zadany level pre ignore_mp_level na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 11:
     ...
     */
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Zle zadany level pre rem_user_maxlevel na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 12:
     ...
     */
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Neznamy level pre rem_user_deflevel na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 13:
    if (strlen(wrd[1]) > VERIFY_LEN) {
      fprintf(stderr, "AngelTalker: Verifikacia je priliz dlha na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 14:
    ...
    if (wrd[1][2] != ':' || strlen(wrd[1]) > 5 || !isdigit(wrd[1][0])
        ...
        || !isdigit(wrd[1][3])
        || !isdigit(wrd[1][4])) {
      fprintf(stderr, "AngelTalker: Chybna sprava overenia casu na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 15:
    if (val < 1) {
      fprintf(stderr, "AngelTalker: Zla hodnota max_users na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    amsys->max_users = val;
  ...
  case 16:
    if (val < 1) {
      fprintf(stderr, "AngelTalker: Zla hodnota heartbeat na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    amsys->heartbeat = val;
  ...
  case 17:
    if (val < 10) {
      fprintf(stderr,
              "AngelTalker: Zla hodnota pre login_idle_time na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 18:
    if (val < 10) {
      fprintf(stderr,
              "AngelTalker: Zla hodnota pre user_idle_time na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 19:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Password_echo musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    amsys->password_echo = val;
    return;
  ...
  case 20:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: Ignore_sigterm musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 21:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Auto_connect musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 22:
    if (val < 0) {
      fprintf(stderr, "AngelTalker: Zadana zla hodnota pre max_clones na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 23:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: Ban_swearing musi byt OFF, MIN alebo MAX na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 24:
    if (!strcmp(wrd[1], "NONE")) {
      ...
      fprintf(stderr,
              "AngelTalker: Crash_action musi byt NONE, SHUTDOWN, REBOOT alebo SEAMLESS na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  ...
  case 25:
    val = onoff_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Colour_def musi byt ON alebo OFF na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 26:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Time_out_afks musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 27:
    val = onoff_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Charecho_def musi byt ON alebo OFF na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 28:
    {
      ...
      if (lvl == NUM_LEVELS) {
        fprintf(stderr,
                "AngelTalker: Neznamy level zadany na time_out_maxlevel na riadku %d.\n",
                config_line);
        boot_exit(1);
      }
  ...
  case 29:
    ...
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Auto_purge musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    if (!val) {
  ...
  case 30:
    ...
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Allow_recaps musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 31:
    ...
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: auto_promote musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 32:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: Personal_rooms musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 33:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Random_motds musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 34:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: Startup_room_parse musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 35:
    ...
    if (val == -1) {
      fprintf(stderr,
              "AngelTalker: Resolve_ip musi byt OFF, AUTO, MANUAL, alebo IDENTD na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 36:
    ...
    val = onoff_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Flood_protect musi byt ON alebo OFF na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  ...
  case 37:
    val = yn_check(wrd[1]);
    if (val == -1) {
      fprintf(stderr, "AngelTalker: Boot_off_min musi byt YES alebo NO na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
...
parse_rooms_section(void)
{
  ...
  if (!strcmp(wrd[0], "ROOMS:")) {
    /* XXX: This could be made idempotent */
    if (in_section++) {
      fprintf(stderr, "AngelTalker: Neocakavana ROOMS section header na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  ...
  if (!*wrd[3]) {
    fprintf(stderr, "AngelTalker: Ocakavany parameter chyba na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[0]) > ROOM_NAME_LEN) {
    fprintf(stderr, "AngelTalker: Meno mapy miestnosti je priliz dlhe na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[1]) > ROOM_LABEL_LEN) {
    fprintf(stderr, "AngelTalker: Nazov miestnosti je priliz dlhy na riadku %d.\n", config_line);
    boot_exit(1);
  }
  if (strlen(wrd[2]) > ROOM_NAME_LEN) {
    fprintf(stderr, "AngelTalker: Meno miestnosti je priliz dlhe na riadku %d.\n", config_line);
    boot_exit(1);
  }
  /* Check for duplicate label or name */
  for (room = room_first; room; room = room->next) {
    if (!strcmp(room->label, wrd[1])) {
      fprintf(stderr, "AngelTalker: Duplikovany nazov miestnosti na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    if (!strcmp(room->name, wrd[2])) {
      fprintf(stderr, "AngelTalker: Duplikovane meno miestnosti na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
  }
  ...
  for (;;) {
    ...
    if (*ptr2 == ',' && !ptr2[1]) {
      fprintf(stderr, "AngelTalker: Chyba nazov linku na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    c = *ptr2;
    *ptr2 = '\0';
    if (!strcmp(ptr1, room->label)) {
      fprintf(stderr, "AngelTalker: Miestnost ma link na samu seba itself na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    ...
    if (++i >= MAX_LINKS) {
      fprintf(stderr, "AngelTalker: Privela linkov na riadku %d.\n", config_line);
      boot_exit(1);
    }
    *ptr2 = c;
    ptr1 = ++ptr2;
  ...
  if (!strcmp(wrd[4], "BOTH")) {
    ...
    fprintf(stderr, "AngelTalker: Neznamy typ pristupu do miestnosti na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  ...
  if (!strcmp(wrd[5], "ACCEPT")) {
    if (*wrd[6] && *wrd[6] != '#') {
      fprintf(stderr,
              "AngelTalker: Neocakavane slovo nasleduje ACCEPT heslo na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    room->inlink = 1;
  ...
  if (!strcmp(wrd[5], "CONNECT")) {
    if (!*wrd[6] || *wrd[6] == '#') {
      fprintf(stderr, "AngelTalker: Meno externeho riadku chyba na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    ...
    if (*wrd[7] && *wrd[7] != '#') {
      fprintf(stderr,
              "AngelTalker: Nezname slovo za externym linkom na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  }
  fprintf(stderr, "AngelTalker: Nezname nastavenie pripojenia na riadku %d.\n",
          config_line);
  boot_exit(1);
#else
  return;
...
parse_topics_section(char *topic)
{
  ...
  if (!strcmp(wrd[0], "TOPICS:")) {
    ...
    if (in_section++) {
      fprintf(stderr,
              "AngelTalker: Neocakavana TOPICS sekcia hlavicky na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  ...
  if (!room) {
    fprintf(stderr, "AngelTalker: Miestnost neexistuje na riadku %d.\n", config_line);
    boot_exit(1);
    return;
  }
...
parse_sites_section(void)
{
  ...
  if (!strcmp(wrd[0], "SITES:")) {
    /* XXX: This could be made idempotent */
    if (in_section++) {
      fprintf(stderr, "AngelTalker: neocakavana SITES cast hlavicky na riadku %d.\n",
              config_line);
      boot_exit(1);
    }
    return;
  ...
  if (!*wrd[3]) {
    fprintf(stderr, "AngelTalker: potrebny parameter chyba na riadku na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[0]) > SERV_NAME_LEN) {
    fprintf(stderr, "AngelTalker: Dlzka linku priliz dlha na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[3]) > VERIFY_LEN) {
    fprintf(stderr, "AngelTalker: Verifikacia priliz dlha na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  nl = create_netlink();
  if (!nl) {
    fprintf(stderr,
            "AngelTalker: Zlyhalo priradenie pamate na vytvorenie netlink na riadku %d.\n",
            config_line);
    boot_exit(1);
    return;
  }
  if (!*wrd[4] || *wrd[4] == '#' || !strcmp(wrd[4], "ALL")) {
    nl->allow = ALL;
    ...
    fprintf(stderr, "AngelTalker: Neznamy netlink access typ na riadku %d.\n",
            config_line);
    boot_exit(1);
  }
  if (strlen(wrd[2]) >= 32) {   /* NI_MAXSERV */
    fprintf(stderr, "AngelTalker: Zle cislo portu na riadku %d.\n", config_line);
    boot_exit(1);
  }
...
/******************************************************************************
                  Inicializacia globalnych a inych veci :-]
                             preklada KvoCo & Week
                               sklonovanie Week
 *****************************************************************************/
...
load_user_details(UR_OBJECT user)
{
  static const char *const userfile_options[] = {
    "version", "password", "promote_date", "times", "levels", "general",
      "user_set",
    "user_ignores", "fighting", "purging", "last_site", "mail_verify",
      "description",
    "in_phrase", "out_phrase", "email", "homepage", "recap_name", "games", "sklonovanie",
    NULL                        /* KEEP HERE! */
  };
  char filename[80];
  char line[ARR_SIZE];
  ...
  const char *str;
  FILE *fp;
  int wcnt, wpos, op, damaged, version_found;
  ...
  for (fgets(line, ARR_SIZE - 1, fp); !feof(fp);
    ...
    for (op = 0; userfile_options[op]; ++op) {
      if (!strcmp(userfile_options[op], user_words[0])) {
        break;
      }
    }
    if (!userfile_options[op]) {
      ++damaged;
      continue;
    }
    switch (op) {
    ...
    case 1:
      if (wcnt >= 2) {
        strcpy(user->pass, remove_first(line));
      }
      break;
    case 2:
      if (wcnt >= 2) {
        strcpy(user->date, remove_first(line));
      }
      break;
    case 3:
      switch (wcnt) {
    ...
    case 4:
      ...
      case 4:
        user->muzzled = atoi(user_words[4]);
      case 3:
        user->arrestby = atoi(user_words[3]);
      case 2:
        user->unarrest = atoi(user_words[2]);
      case 1:
        user->level = atoi(user_words[1]);
      case 0:
        break;
      }
      break;
    ...
    case 12:
      if (wcnt >= 2) {
        strcpy(user->desc, remove_first(line));
      }
      break;
    case 13:
      if (wcnt >= 2) {
        strcpy(user->in_phrase, remove_first(line));
      }
      break;
    case 14:
      if (wcnt >= 2) {
        strcpy(user->out_phrase, remove_first(line));
      }
      break;
    case 15:
      if (wcnt >= 2) {
    ...
    case 17:
      if (wcnt >= 2) {
        strcpy(user->recap, remove_first(line));
      }
      break;
    case 18:
      switch (wcnt) {
      ...
      default:
      case 6:
        strcpy(user->instr, user_words[6]);
      case 5:
        strcpy(user->lokal, user_words[5]);
      case 4:
        strcpy(user->akuza, user_words[4]);
      case 3:
        strcpy(user->dativ, user_words[3]);
      case 2:
        strcpy(user->genit, user_words[2]);
      case 1:
        strcpy(user->nomin, user_words[1]);
      case 0:
        break;
      }
      break;
  ...
  if (!version_found) {
    ...
    write_syslog(SYSLOG, 1, "POSKODENE userfile \"%s.D\"\n", user->name);
  }
...
save_user_details(UR_OBJECT user, int save_current)
{
  ...
  fp = fopen(filename, "w");
  if (!fp) {
    vwrite_user(user, "%s: nemozem zapisat tvoj subor.\n", syserror);
    write_syslog(SYSLOG, 1, "SAVE_USER_STATS: Nemozno ulozit %s.\n",
                 user->name);
    return 0;
  }
  /* reset normal level */
  ...
  if (save_current) {
    ...
    fprintf(fp, "times         %d %d %d %d\n", (int) user->last_login,
            (int) user->total_login, user->last_login_len,
            (int) user->read_mail);
  }
  fprintf(fp, "levels        %d %d %d %d %d\n", user->level, user->unarrest,
          user->arrestby, user->muzzled, user->retired);
  fprintf(fp, "general       %d %d %d %d %d %d %d %d\n", user->accreq,
          user->charmode_echo, user->command_mode, user->prompt, user->vis,
          user->monitor, user->mail_verified, user->logons);
  fprintf(fp, "user_set      %d %d %d %d %d %d %d %d %d %d %d %d %s %d\n",
          user->gender, user->age, user->wrap, user->pager, user->hideemail,
          user->colour, user->lroom, user->alert, user->autofwd,
          user->show_pass, user->show_rdesc, user->cmd_type,
          *user->icq ? user->icq : "#UNSET", user->reverse_buffer);
  fprintf(fp, "user_ignores  %d %d %d %d %d %d %d %d\n", user->ignall,
          user->igntells, user->ignshouts, user->ignpics, user->ignlogons,
  ...
  if (!save_current) {
    ...
    if (user->level == NEW) {
      fprintf(fp, "purging       %d %d\n", user->expire,
              (int) (time(0) + (NEWBIE_EXPIRES * 86400)));
    } else {
      fprintf(fp, "purging       %d %d\n", user->expire,
              (int) (time(0) + (USER_EXPIRES * 86400)));
    }
  }
  if (save_current) {
  ...
  fprintf(fp, "games         %d %d %d %d %d %d %d %d\n",
          user->hits, user->misses, user->deaths, user->kills, user->bullets,
          user->hps, user->money, user->bank);
  fprintf(fp, "sklonovanie   %s %s %s %s %s %s\n",
          user->nomin, user->genit, user->dativ, user->akuza, user->lokal,
          user->instr);
  fclose(fp);
  return 1;
}
...
load_user_details_old(UR_OBJECT user)
{
  ...
  fscanf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", &temp1,
         ...
         &user->monitor, &user->unarrest, &user->logons, &user->accreq,
...
load_oldversion_user(UR_OBJECT user, int version)
{
  ...
  if (version >= 200) {
    ...
    fscanf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", &temp1,
           ...
           &user->monitor, &oldvote, &user->unarrest, &user->logons,
...
process_users(void)
{
  ...
  if (!dirp) {
    fprintf(stderr, "AngelTalker: Otvorenie adresara zlyhalo v process_users().\n");
    boot_exit(12);
  }
  u = create_user();
  if (!u) {
    fprintf(stderr, "AngelTalker: Vytvorenie uzivatela zlyhalo v process_users().\n");
    closedir(dirp);
    boot_exit(17);
    return;
  }
  ...
  if (dp) {
    fprintf(stderr,
            "AngelTalker: Nemozem nacitat userfile \"%s\" v process_users().\n",
            name);
    boot_exit(18);
  }
}
...
count_users(void)
{
  ...
  amsys->user_count = 0;
  for (lvl = JAILED; lvl < NUM_LEVELS; ++lvl) {
    amsys->level_count[lvl] = 0;
  }
  /* count up users */
  for (entry = first_user_entry; entry; entry = entry->next) {
...
parse_commands(void)
{
  ...
  for (cnt = 0; command_table[cnt].name; ++cnt) {
    if (!add_command(cnt)) {
      fprintf(stderr,
              "AngelTalker: Zlyhalo priradenie pamate pre parse_commands().\n");
      boot_exit(13);
    }
  }
  return;
...
/******************************************************************************
         Suborove funkcie - citanie, zapisovanie, pocitanie riadkov, atd
                           S prekladom sa jebal KvoCo
 *****************************************************************************/
...
remove_top_bottom(char *filename, int where)
{
  ...
  if (!where) {
    ...
    /* get rest of file */
    for (fgets(line, ARR_SIZE, fpi); !feof(fpi);
    ...
    --cnt;
    i = 0;
    for (fgets(line, ARR_SIZE - 1, fpi); !feof(fpi);
         fgets(line, ARR_SIZE - 1, fpi)) {
      if (i++ < cnt) {
        line[strlen(line) - 1] = '\0';
        fprintf(fpo, "%s\n", line);
...
/******************************************************************************
 Zapisovacia funkcie - uzivatelov, miestnosti, systemovych logov
 *****************************************************************************/
 ...
 */
void
record_last_command(UR_OBJECT user, enum cmd_value comnum, size_t len)
{
  char dstr[32];
  time_t now;
  ...
  sprintf(cmd_history[amsys->last_cmd_cnt & 15],
          ...
          command_table[comnum].name, (int) len, word_count, user->name);
  ++amsys->last_cmd_cnt;
}
...
write_monitor(UR_OBJECT user, RM_OBJECT rm, int rec)
{
  ...
  for (u = user_first; u; u = u->next) {
    ...
    if ((!cmd || u->level < cmd->min_lev) && !has_gcom(u, MONITOR)) {
      continue;
    }
    if (u->room == rm || !rm) {
      if (!u->ignall) {
...
more_users(UR_OBJECT user)
{
  ...
  if (entry) {
    write_user(user, "~BB~FG-=[*]=- STLAC <ENTER>, E PRE KONIEC:~RS ");
    no_prompt = 1;
    user->misc_op = 16;
    return 1;
  }
...
add_history(char *name, int showtime, const char *str, ...)
{
  ...
  *name = toupper(*name);
  ...
  va_end(args);
  fclose(fp);
  write_syslog(SYSLOG, 1, "HISTORY pridana pre %s.\n", name);
}



/******************************************************************************
 Prihlasovacie a odhlasovacie funkcie
 prelozil KvoCo
 *****************************************************************************/
...
login(UR_OBJECT user, char *inpstr)
{
  ...
  *passwd = '\0';
  ...
  case LOGIN_NAME:
    ...
    if (i > USER_NAME_LEN) {
      write_user(user, "\nTy koki, jaky dlhy nick... skus neco kratsie.\n\n");
      attempts(user);
      return;
    }
    if (contains_swearing(name)) {
      write_user(user, "\nPrepac, ale takyto nick nemozes pouzit!\n\n");
      attempts(user);
      return;
    }
    strtolower(name);
    *name = toupper(*name);
    if (user_banned(name)) {
      write_user(user, "\nTento nick je zabanovany...\n\n");
      disconnect_user(user);
      write_syslog(SYSLOG, 1, "Zabanovany user %s sa pokusil nalogovat.\n", name);
      return;
    }
    ...
    if (!load_user_details(user)) {
      ...
      if (user->wizport) {
        write_user(user,
                   "\nLutujem, na tomto porte nemozes vytvarat nove konta.\n\n");
        disconnect_user(user);
        return;
      }
#endif
      if (amsys->minlogin_level != NUM_LEVELS) {
        write_user(user,
                   "\nLutujem, prave sa nedaju vytvorit nove konta.\n\n");
        disconnect_user(user);
        return;
      }
      if (site_banned(user->site, 1)) {
        write_user(user,
                   "\nLutujem, ale nove konta boli z tvojej adresy zakazane.\n\n");
        write_syslog(SYSLOG, 1,
                     "Pokus o login zo zakazanej adresy >>> %s.\n",
                     user->site);
        disconnect_user(user);
        return;
      }
      write_user(user, "Novy uzivatel...\n");
    } else {
#ifdef WIZPORT
      if (user->wizport && user->level < amsys->wizport_level) {
        vwrite_user(user,
                    "\nLutujem, na tento port sa mozu nalogovat len useri s levelom %s a vyssim.\n\n",
      ...
      if (amsys->minlogin_level != NUM_LEVELS
        ...
        write_user(user,
                   "\nLutujem, ludia s tvojim levelom maju mmntalne zakazany pristup na talker.\n\n");
        disconnect_user(user);
        return;
      }
    }
    write_user(user, "Heslo: ");
    echo_off(user);
  ...
  case LOGIN_PASSWD:
    ...
    if (!*user->pass) {
      ...
      if (i > 8) {
        write_user(user,
                   "\n\nVAROVANIE: Zo zadaneho hesla bude pouzitych len prvych 8 znakov!\n\n");
      }
      ...
      if (more(NULL, user->socket, filename)) {
        write_user(user,
                   "\nPred potvrdenim hesla si precitaj pravidla.\n");
        write_user(user,
                   "Ak s nimi nesuhlasis tak sa ihned odhlas.\n");
      }
      write_user(user, "\nPotvrd heslo: ");
      user->login = LOGIN_CONFIRM;
    } else {
      if (strcmp(user->pass, crypt(passwd, user->pass))) {
        write_user(user, "\n\nNespravne prihlasenie.\n\n");
        attempts(user);
        return;
      }
      echo_on(user);
      ...
      write_user(user, "Stlac <ENTER> pre pokracovanie: ");
  ...
  case LOGIN_CONFIRM:
    sscanf(inpstr, "%s", passwd);
    if (strcmp(user->pass, crypt(passwd, user->pass))) {
      write_user(user, "\n\nHeslo viacmenej nepasuje...\n\n");
      attempts(user);
      return;
    }
    echo_on(user);
    strcpy(user->desc, "este nema nastaveny desc.");
    strcpy(user->in_phrase, "vosiel");
    strcpy(user->out_phrase, "odisiel");
    strcpy(user->date, (long_date(1)));
    strcpy(user->recap, user->name);
    strcpy(user->bw_recap, colour_com_strip(user->recap));
     strcpy(user->nomin, user->name);
     strcpy(user->genit, "tvora_");
     strcpy(user->dativ, "tvorovi_");
     strcpy(user->akuza, "tvora_");
     strcpy(user->lokal, "tvorovi_");
     strcpy(user->instr, "tvorom_");
     strcat(user->genit, user->name);
     strcat(user->dativ, user->name);
     strcat(user->akuza, user->name);
     strcat(user->lokal, user->name);
     strcat(user->instr, user->name);
    *user->last_site = '\0';
    user->level = NEW;
    user->unarrest = NEW;
    user->muzzled = 0;
    ...
    add_history(user->name, 1, "Bol vytvoreny.\n");
    write_syslog(SYSLOG, 1, "Vytvoreny novy uzivatel - \"%s\".\n", user->name);
    ++amsys->logons_new;
...
attempts(UR_OBJECT user)
{
  ...
  if (user->attempts == LOGIN_ATTEMPTS) {
    write_user(user, "\nLutujem, prihlaseny maximalny pocet userof.\n\n");
    disconnect_user(user);
    return;
  }
  reset_user(user);
...
show_login_info(UR_OBJECT user)
{
  ...
  static const char *const times[] = { "Bre ranko", "Pekny den prajem", "Bry vecer" };
  ...
  sprintf(text, "%s, %s~RS, vitaj na %s talkri", times[phase],
          user->recap, TALKER_NAME);
  cnt = 74 + teslen(text, 74);
  vwrite_user(user, "~FB+~FW %-*.*s ~FB+\n", cnt, cnt, text);
  sprintf(text, "Je prave ~OL%s~RS", long_date(1));
  cnt = 74 + teslen(text, 74);
  vwrite_user(user, "~FB+~FW %-*.*s ~FB+\n", cnt, cnt, text);
  write_user(user,
             "\n~FB.+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n");
  if (*user->last_site) {
    ...
    sprintf(text, "Naposledy prihlaseny : ~OL%-25s~RS   Tvoj level : ~OL%s~RS",
            temp, user_level[user->level].name);
    cnt = 74 + teslen(text, 74);
    vwrite_user(user, "~FB+~FW %-*.*s ~FB+\n", cnt, cnt, text);
    sprintf(text, "Naposledy z adresy : ~OL%s~RS", user->last_site);
    cnt = 74 + teslen(text, 74);
    vwrite_user(user, "~FB+~FW %-*.*s ~FB+\n", cnt, cnt, text);
    ++exline;
  }
  if (user->level >= (enum lvl_value) command_table[INVIS].level) {
    sprintf(text, "Tvoj visible stav -  %s~RS", see[user->vis]);
    if (user->level >= (enum lvl_value) command_table[MONITOR].level) {
      sprintf(text2, " a monitoring je %s~RS", myoffon[user->monitor]);
      strcat(text, text2);
    } else {
      strcat(text, "\n");
    }
    ...
    sprintf(text, "Tvoj monitoring je prave %s~RS", myoffon[user->monitor]);
    cnt = 74 + teslen(text, 74);
    vwrite_user(user, "~FB+~FW %-*.*s ~FB+\n", cnt, cnt, text);
    ++exline;
  }
  ...
  *temp = '\0';
  ...
  if (!yes) {
    ...
    if (user->ignshouts) {
      strcat(text2, "Shout-y   ");
      ++yes;
    }
    if (user->ignpics) {
      strcat(text2, "Obrazky   ");
      ++yes;
    }
    if (user->ignlogons) {
      strcat(text2, "Logon-y   ");
      ++yes;
    }
    if (user->ignwiz) {
    ...
    if (user->igngreets) {
      strcat(text2, "Pozdravy   ");
      ++yes;
    }
    if (user->ignbeeps) {
      strcat(text2, "Peepanie");
      ++yes;
    }
  }
  if (yes) {
  ...
  if (user->level >= (enum lvl_value) command_table[REMINDER].level) {
    ...
    sprintf(text, "Nadnes mas ~OL%d~RS pripomienok", cnt);
    vwrite_user(user, "| %-80.80s |\n", text);
    cnt = remove_old_reminders(user);
    if (cnt) {
      sprintf(text,
              "Mas ~OL%d~RS pripomenienok odstranenich pre vyprsanie platnosti",cnt);
      vwrite_user(user, "| %-80.80s |\n", text);
    }
    write_user(user,
               "~FB.+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..\n");
...
connect_user(UR_OBJECT user)
{
  ...
  if (u) {
...
#endif
    {
      ...
      write_user(user,
                 "\n\nUz si raz prihlaseny...prepinam na staru session...\n");
  ...
  if (user->vis) {
    ...
    vwrite_level(ARCH, 1, NORECORD, user,
                 "%s~RS ~FC(Adresa %s : User Port %s : Talker Port %s)\n",
    ...
    vwrite_level(user->level < ARCH ? ARCH : user->level, 1, NORECORD, user,
                 "~OL~FY[ INVIS ]~RS %s~RS ~FC(Adresa %s : User port %s : Talker Port : %s)\n",
                 user->recap, user->site, user->site_port,
                 !user->wizport ? amsys->mport_port : amsys->wport_port);
#else
    vwrite_level(user->level < ARCH ? ARCH : user->level, 1, NORECORD, user,
  ...
  /*som dal prec, aby pri starte vypisovalo kolko mas celkove mailof v schranke... Week */
  ...
  if (user->accreq != -1 && amsys->auto_promote) {
    vwrite_user(user,
                ...
                bp);
    write_user(user,
               "~OL~FY*                   ~FRCITAJ POZORNE PRE AUTO-PROMOTE~FY                   *\n");
    write_user(user,
               "~OL~FY* Musis si nastavit tvoj desc (strucny popis - .desc), nastavit pohlavie   *\n");
    write_user(user,
               "~OL~FY* (.set gender) a pouzi .accreq prikaz, az potom ti bude udelene POVYSENIE *\n");
    write_user(user,
               "~OL~FY****************************************************************************\n\n");
  }
  prompt(user);
...
disconnect_user(UR_OBJECT user)
{
  ...
  if (user->type != REMOTE_TYPE) {
    ...
    write_user(user, "\n~OL~FBOpustil si Angel talker, teraz ti uz nikto nepomoze...\n\n");
    vwrite_user(user, "Visel si zo sajty %s\n", user->site);
    strftime(dstr, 32, "%a %Y-%m-%d %H:%M:%S", localtime(&now));
    vwrite_user(user, "Dna %s, celkovo si odwisel %d hodin a %d minut.\n\n",
                dstr, (int) hours, (int) mins);
    shutdown(user->socket, SHUT_WR);
    close(user->socket);
    logon_flag = 1;
    if (user->vis) {
      vwrite_room(NULL, "~OL[Odisiel:~RS %s~RS %s~RS~OL]\n", user->recap,
                  user->desc);
    } else {
      vwrite_level(WIZ, 1, NORECORD, NULL,
                   "~OL~FY[ INVIS ]~RS ~OL[Odisiel:~RS %s~RS %s~RS~OL]\n",
                   user->recap, user->desc);
    }
    logon_flag = 0;
#ifdef NETLINKS
...
misc_ops(UR_OBJECT user, char *inpstr)
{
    ...
    if (tolower(*inpstr) == 'y') {
      if (amsys->rs_countdown && !amsys->rs_which) {
        if (amsys->rs_countdown > 60) {
          vwrite_room(NULL,
                      "\n\07~OLSYSTEM: ~FR~LISPUSTENY SHUTDOWN, za cas %d minuty, %d sekund!\n\n",
                      amsys->rs_countdown / 60,
                      amsys->rs_countdown % 60);
        } else {
          vwrite_room(NULL,
                      "\n\07~OLSYSTEM: ~FR~LISPUSTENY SHUTDOWN, talker sa vypne za %d sekund!\n\n",
                      amsys->rs_countdown);
        }
        write_syslog(SYSLOG, 1,
                     "%s spustil(a) -- %d sekundovy SHUTDOWN countdown.\n",
    ...
    if (tolower(*inpstr) == 'y') {
      if (amsys->rs_countdown && amsys->rs_which > 0) {
        if (amsys->rs_which == 1) {
          if (amsys->rs_countdown > 60) {
            vwrite_room(NULL,
                        "\n\07~OLSYSTEM: ~FY~LISpusteny REBOOT, za %d minut, %d sekund!\n\n",
                        amsys->rs_countdown / 60,
                        amsys->rs_countdown % 60);
          } else {
            vwrite_room(NULL,
                        "\n\07~OLSYSTEM: ~FY~LISpusteny REBOOT, za %d sekund!\n\n",
                        amsys->rs_countdown);
          }
        } else {
          if (amsys->rs_countdown > 60) {
            vwrite_room(NULL,
                        "\n\07~OLSYSTEM: ~FY~LISpusteny SEAMLESS REBOOT, reboot za %d minut, %d sekund!\n\n",
                        amsys->rs_countdown / 60,
                        amsys->rs_countdown % 60);
          } else {
            vwrite_room(NULL,
                        "\n\07~OLSYSTEM: ~FY~LISpusteny SEAMLESS REBOOT, reboot za %d sekund!\n\n",
                        amsys->rs_countdown);
          }
        }
        write_syslog(SYSLOG, 1, "%s spustil %d sekundovy %s countdown.\n",
                     user->name, amsys->rs_countdown,
  ...
  case 18:
    if (tolower(*inpstr) == 'y') {
      ...
      remove(filename);
      write_user(user, "\n~OL~FRVsetky spravy vymazane.\n\n");
    } else {
      write_user(user, "\nNeboli vymazane ziadne spravy.\n\n");
    }
    user->misc_op = 0;
    return 1;
...
{
  char filename[80], *comword = NULL, possible[ARR_SIZE], code1[6], code2[6];
  CMD_OBJECT cmd;
  size_t i, len;
  ...
  if (!*comword) {
    write_user(user, "Uff.. taky prikaz nepoznam..\n");
    return 0;
  }

  len = strlen(comword);
  for (com_num = 0; command_table[com_num].name; ++com_num) {
    if (!strncmp(command_table[com_num].name, comword, len)) {
      break;
    }
  }

  /* If command could not be found, try some soundex matching */
  if (user->room && !command_table[com_num].name) {
    get_soundex(comword, code1);
    *code2 = '\0';
    *possible = '\0';
    for (i = 0; command_table[i].name; ++i) {
      if ((enum lvl_value) command_table[i].level <= user->level) {
        get_soundex(command_table[i].name, code2);
        if (!strcmp(code1, code2)) {
          strcat(possible, "  ");
          strcat(possible, command_table[i].name);
        }
      }
    }
    write_user(user, "Uff.. taky prikaz nepoznam...\n");
    if (strlen(possible)) {
      vwrite_user(user, "Mozno niektory z tychto:%s\n", possible);
    }
    return 0;
  }
  ...
  /*
     You may wonder why I am using com_num above and then scrolling through the command
  ...
  for (cmd = first_command; cmd; cmd = cmd->next) {
    if ((enum cmd_value) cmd->id == com_num) {
      if (user->room && (has_gcom(user, cmd->id))) {
        ++cmd->count;
        break;
      }
      if (user->room && (has_xcom(user, cmd->id))) {
        write_user(user, "Tento prikaz prave nomozes pouzivat.\n");
        return 0;
      }
      if (user->room && (cmd->min_lev > user->level)) {
        vwrite_user(user, "Prikaz mozu pouzit ludia s levelom [~OL%s~RS] a vyssim.\n", user_level[cmd->min_lev].name);
        return 0;
      }
      ++cmd->count;
      break;
...
look(UR_OBJECT user)
{
  ...
  if (is_private_room(rm)) {
    char pred [100] = "~FY.+'`'+. ~FGLokacia: ~OL~FR", vysled [100] = "" ,medzi [100] = "" , 
    po []  = " ~RS~FY.+'`'+..+'`'+..+'`'+..+'`'+..+'``'+..+'`'+..+'`'+..+'`'+..+'`'+..+'``'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`";
    strcat(medzi,pred);
    strcat(medzi,(is_personal_room(rm) ? rm->show_name : rm->name));
    strcat(medzi,po);
    
    strncpy(vysled, medzi, KOLKOMAX);

  
    vwrite_user(user,"\n~RS%s~RS\n\n",vysled);
  } else {
    char pred [100] = "~FY.+'`'+. ~FGLokacia: ~OL~FW", vysled [100] = "" ,medzi [100] = "" , 
    po []  = " ~RS~FY.+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`'+..+'``'+..+'`'+..+'`'+..+'`'+..+'`'+..+'`";
    strcat(medzi,pred);
    strcat(medzi,(is_personal_room(rm) ? rm->show_name : rm->name));
    strcat(medzi,po);
  ...
  for (u = user_first; u; u = u->next) {
    ...
    if (!users++) {
      write_user(user, "~FG~OLOkrem teba su tu:\n");
    }
    vwrite_user(user, "     %s%s~RS %s~RS  %s\n", u->vis ? " " : "~FR*~RS",
                u->recap, u->desc, u->afk ? "~BR(AFK)" : "");
  }
  if (!users) {
    write_user(user, "~FGSi tu sam.\n");
  }
  write_user(user, "\n");
  ...
  if (is_personal_room(rm)) {
    strcat(text, "osobny ");
    if (is_private_room(rm)) {
      strcat(text, "~FR(zamkuty)~RS");
    } else {
      strcat(text, "~FG(odomknuty)~RS");
    }
  } else {
    if (is_fixed_room(rm)) {
      strcat(text, "~FRfixovany~RS na ");
    } else {
      strcat(text, "nastaveny na ");
    }
    if (is_private_room(rm)) {
      strcat(text, "~FRPRIVATNY~RS");
    } else {
      strcat(text, "~FGVEREJNY~RS");
    }
  }
  sprintf(temp," a sprav na nastenke : ~OL~FM%d~RS.\n", rm->mesg_cnt);
  /* , --are,is , PLTEXT_S(rm->mesg_cnt) --'s' plural */
  strcat(text, temp);
  write_user(user, text);
  if (*rm->topic) {
    vwrite_user(user, "~FG~OLAktualna tema je:~RS %s\n", rm->topic);
  } else {
    write_user(user, "~FGTema nenastavena.\n");
  }
}
...
help(UR_OBJECT user)
{
  ...
  *text = '\0';
  for (i = 0; command_table[i].name; ++i) {
    if (!strncasecmp(command_table[i].name, word[1], len)) {
      if (strlen(command_table[i].name) == len) {
	break;
      }
      /* FIXME: take into account xgcoms, command list dynamic level, etc. */
      if (user->level < (enum lvl_value) command_table[i].level) {
	continue;
      }
      strcat(text, found++ % 8 ? "  " : "\n  ~OL");
  ...
  *text = '\0';
  if (!found) {
    write_user(user, "Sorac, s tymto ti nepomozem.\n");
    return;
  }
  if (word_count < 3) {
    sprintf(filename, "%s/%s", HELPFILES, command_table[c].name);
  } else {
    if (SET == c) {
      int a;
	...
	if (!strncasecmp(setstr[i].type, word[2], len)) {
	  if (strlen(setstr[i].type) == len) {
	    break;
	  }
	  strcat(text, found++ % 8 ? "  " : "\n  ~OL");
	  strcat(text, setstr[i].type);
	  a = i;
	}
      }
      if (setstr[i].type) {
	...
	*text = '\0';
	...
	return;
      ...
      if (word_count < 4) {
	sprintf(filename, "%s/%s_%s", HELPFILES, command_table[c].name, setstr[a].type);
      } else {
	sprintf(filename, "%s/%s", HELPFILES, command_table[c].name);
      }
    } else {
  ...
  switch (more(user, user->socket, filename)) {
    ...
    case 2:
      /* FIXME: take into account xgcoms, command list dynamic level, etc. */
      vwrite_user(user, "~OLLevel   :~RS %s a vyssi\n", user_level[command_table[c].level].name);
      break;
  }
}
...
help_commands_level(UR_OBJECT user)
{
  ...
  for (lvl = JAILED; lvl < NUM_LEVELS; ++lvl) {
    ...
    cnt = 0;
    *text = '\0';
    sprintf(text, "  ~FG~OL%c)~RS ~FC", user_level[lvl].name[0]);
    highlight = 1;
...
help_commands_function(UR_OBJECT user)
{
  ...
  for (function = 0; command_types[function]; ++function) {
    ...
    *text = '\0';
    ...
    for (cmd = first_command; cmd; cmd = cmd->next) {
      *temp1 = '\0';
      if (cmd->min_lev > user->level || cmd->function != function) {
        continue;
      }
      if (!found++) {
        write_user(user,
...
help_amnuts_credits(UR_OBJECT user)
{
  write_user(user,
             "~BM             ~BB             ~BC             ~BG             ~BY             ~BR             ~RS\n\n");
  vwrite_user(user,
              "~OL~FCAmnuts version %s~RS, Copyright (C) Andrew Collington, 2001\n\n",
              AMNUTSVER);
  write_user(user,
             "Amnuts stands for ~OLA~RSndy's ~OLM~RSodified ~OLNUTS~RS, a Unix talker server written in C.\n");
  write_user(user,
             "Many thanks to everyone who has used the Amnuts code, website, or has bought\n");
  write_user(user,
             "any of the Amnuts merchandise.  Special thanks for this version go to my wife,\n");
  write_user(user,
             "Lisa, Ardant for bug fixes and numerous code additions, Silver (of PG+ fame) for\n");
  write_user(user,
             "Makefiles and additional code from PG+, and Arny for his seamless reboot code.\n");
  write_user(user,
             "But, really, thanks to everyone who has used Amnuts in one way or another,\n");
  write_user(user,
             "because without all you, Amnuts would just be nothing.\n");
  write_user(user,
  ...
  write_user(user,
             "free to email me at ~OL~FCamnuts@talker.com~RS.  The Amnuts website, which \n");
  write_user(user,
             "has a message board, faqs, and more, is at  ~OL~FChttp://amnuts.talker.com/~RS\n");
  write_user(user,
             "I hope you enjoy the talker!\n\nAndrew Collington, September 2001\n\n(for NUTS credits, see \".help nuts\")\n");
  write_user(user,
             "\n~BM             ~BB             ~BC             ~BG             ~BY             ~BR             ~RS\n\n");
}
...
status(UR_OBJECT user)
{
  ...
  if (!on || u->login) {
    ...
    vwrite_user(user,
                ...
                ir, noyes[(u->muzzled > 0)], noyes[u->ignall]);
#ifdef NETLINKS
...
set_attributes(UR_OBJECT user)
{
  ...
  if (i == -1) {
    write_user(user, "Sucastne nastavenia:\n\n");
    for (i = 0; setstr[i].type; ++i) {
  ...
  case SETGEND:
    *word[2] = tolower(*word[2]);
    if (*word[2] == 'm' || *word[2] == 'z') {
      switch (*word[2]) {
      case 'm':
        user->gender = MALE;
        write_user(user, "Pohlavie nastavene na muzske\n");
	    /*ono to sice vypada zlozito... ale jednoduchsie ma nic nenapadlo,
	      bez toho, aby mi to nesalelo user->name*/
        strcpy(user->nomin, user->name);
	
	strcpy(user->genit, genitiv_m(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->dativ, dativ_m(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->akuza, akuzativ_m(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->lokal, lokal_m(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->instr, instrumental_m(user->nomin));
	strcpy(user->nomin, user->name);
	break;
      case 'z':
        user->gender = FEMALE;
        write_user(user, "Pohlavie nastavene na zenske\n");
        strcpy(user->nomin, user->name);
	strcpy(user->genit, genitiv_z(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->dativ, dativ_z(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->akuza, akuzativ_z(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->lokal, lokal_z(user->nomin));
	strcpy(user->nomin, user->name);
	strcpy(user->instr, instrumental_z(user->nomin));
	strcpy(user->nomin, user->name);
	break;
      }
    ...
    write_user(user, "Usage: set gender m|z\n");
    return;
  case SETAGE:
    if (word_count < 3) {
  ...
  case SETRECAP:
    if (!amsys->allow_recaps) {
      write_user(user,
                 "Sorac, recap je prave zakazany.\n");
      return;
    }
    if (word_count < 3) {
      write_user(user, "Pouzitie: set recap <name as you would like it>\n");
      return;
    }
...
check_macros(UR_OBJECT user, char *inpstr)
{
  ...
  if (*inpstr == '.' && inpstr[1] >= '0' && inpstr[1] <= '9') {
#ifdef NETLINKS
    if (user->type == REMOTE_TYPE) {
    ...
    *line = '\0';
    if (inpstr[2]) {
      strncat(line, inpstr + 3, ARR_SIZE - 1);
    }
...
user_listen(UR_OBJECT user)
{
  ...
  if (user->vis) {
    vwrite_room_except(user->room, user,
                       "%s is now listening to you all again.\n", user->name);
  }
}
...
show_last_login(UR_OBJECT user)
{
  ...
  if (word_count == 2) {
    ...
    write_user(user,
               "\n+----------------------------------------------------------------------------+\n");
    vwrite_user(user, "| ~FC~OLLast login details of %-52s~RS |\n", u->name);
    write_user(user,
               "+----------------------------------------------------------------------------+\n");
...
display_colour(UR_OBJECT user)
{
  ...
  for (i = 0; colours[i]; ++i) {
    vwrite_user(user, "~%s: %sAngel %s color preview~RS\n",
                colours[i], colours[i], ANGELVER);
  }
}
...
show_ranks(UR_OBJECT user)
{
  ...
  for (cmd = 0; command_table[cmd].name; ++cmd) {
    ++cnt[command_table[cmd].level];
  }
  write_user(user,
             "+----------------------------------------------------------------------------+\n");
  write_user(user,
...
wiz_list(UR_OBJECT user)
{
  ...
  if (!count) {
    write_user(user, "Je mi luto, ale momentalne nie su ONLINE ziadni wizzardi...\n");
  }
  write_user(user, "\n");
  write_user(user,
             "+----------------------------------------------------------------------------+\n");
...
get_time(UR_OBJECT user)
{
  ...
  vwrite_user(user, "| Angel nabootovany          : ~OL%-45s~RS |\n", dstr);
...
show_version(UR_OBJECT user)
{
  ...
  write_user(user,
             "+----------------------------------------------------------------------------+\n");
  vwrite_user(user,
              "| ~FC~OLAmnuts version %5s                 (C) Andrew Collington, September 2001~RS |\n",
              AMNUTSVER);
  vwrite_user(user,
              "| ~FC~OLBuilt %s %s~RS                                                 |\n",
              __DATE__, __TIME__);
...
show_command_counts(UR_OBJECT user)
{
  ...
  *text2 = '\0';
  ...
  write_user(user,
             "\n+----------------------------------------------------------------------------+\n");
  write_user(user,
             "| ~FC~OLStatistiky pouzivanie prikazov~RS                                             |\n");
  write_user(user,
             "+----------------------------------------------------------------------------+\n");
  for (cmd = first_command; cmd; cmd = cmd->next) {
...
alert_friends(UR_OBJECT user)
{
  ...
  for (u = user_first; u; u = u->next) {
    ...
    if (user_is_friend(u, user) && user->vis) {
      vwrite_user(u,
                  "\n\07~FG~OL~LIHOPPA!~RS~OL~FG Tvoj kamarat ~FC%s~FG sa prave prihlasil\n\n",
                  user->name);
    }
  }
}
  ...
  *text2 = '\0';
  ...
  if (word_count < 2) {
    if (!count_friends(user)) {
      write_user(user, "Nemas nikoho vo friends liste.\n");
      return;
    }
    for (fu = user->fu_first; fu; fu = fu->next) {
      if (fu->flags & fufFRIEND) {
...
"Nov", "Dec" };
  ...
  if (word_count > 3 || yr > 3000 || yr < 1800 || mo < 1 || mo > 12) {
    write_user(user, "Pouzitie: calendar [<m> [<r>]]\n");
    write_user(user, "where: <m> = mesiac od 1 do 12\n");
    write_user(user, "       <r> = rok od 1 do 99, alebo od 1800 do 3000\n");
    return;
  }
  /* show calendar */
  write_user(user, "\n+-----------------------------------+\n");
...
read_user_reminders(UR_OBJECT user)
{
  ...
  i = 0;
  ln = 0;
  for (fgets(line, REMINDER_LEN, fp); !(feof(fp));
...
/*********************************** NAKE FCIE NA SKLONOVANIE *************************************/

//nominativ je eqal
char *
nominativ_m(char meno[USER_NAME_LEN+5])
	{
	return meno;
	}

// genitiv... u muzskeho pohlavia ;))

char *
genitiv_m(char meno[USER_NAME_LEN+5])
	{
	if (meno[strlen(meno)-1] == 97 || meno[strlen(meno)-1] == 101 || meno[strlen(meno)-1] == 105 || meno[strlen(meno)-1] == 111 || meno[strlen(meno)-1] == 117)
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
	    strncpy(pomocna,meno,(strlen(meno)-1));
	    strcat(pomocna,"a");
	    strcpy(meno,pomocna);
	    return meno;
	    }
	if (meno[strlen(meno)-1] == 121 || meno[strlen(meno)-1] == 105)
	    {
	    strcat(meno, "ho");
	    return meno;
	    }
...
// dativ muzske pohlavie....

char *
dativ_m(char meno[USER_NAME_LEN+5])
	{
	if (meno[strlen(meno)-1] == 111)
	    {
	    strcat(meno, "vi");
	    return meno;
	    }
...
// akuzativ muzske pohlavie ...

char *
akuzativ_m(char meno[USER_NAME_LEN+5])
	{
	if (meno[strlen(meno)-1] == 111 )
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
...
// lokal muzske pohlavie

char *
lokal_m(char meno[USER_NAME_LEN+5])
	{
	if (meno[strlen(meno)-1] == 111)
	    {
	    strcat(meno, "vi");
...
char *
instrumental_m(char meno[USER_NAME_LEN+5])
	{
	if (meno[strlen(meno)-1] == 111)
	    {
	    strcat(meno, "m");
...
// GOGOGO na zenske......
char *
nominativ_z(char meno[USER_NAME_LEN+5])
	{
	return meno;
	}
...
char *
genitiv_z(char meno[USER_NAME_LEN+5])
	{
	
	//oki, takze checknem najprv ci je posledne a-cko ak hej prasknem ho het....
	if (meno[strlen(meno)-1] == 97)
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
	    strncpy(pomocna,meno,(strlen(meno)-1));
	    strcpy(meno,pomocna);
	...
	if (meno[strlen(meno)-1] == 111 || meno[strlen(meno)-1] == 105 || meno[strlen(meno)-1] == 117 || meno[strlen(meno)-1] == 121 || meno[strlen(meno)-1] == 99)
	    {
	    strcat(meno, "e");
	    return meno;
	    }
	
	if (meno[strlen(meno)-1] == 106)
	    {
	    strcat(meno, "i");
	    return meno;
	    }
...
char *
dativ_z(char meno[USER_NAME_LEN+5])
	{
	
	//eqal jak genitiv zmena koncoviek....
	if (meno[strlen(meno)-1] == 97)
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
	    strncpy(pomocna,meno,(strlen(meno)-1));
	...
	//sklonovanie podla vzoru ulica....
	if (meno[strlen(meno)-1] == 106 || meno[strlen(meno)-1] == 111 || meno[strlen(meno)-1] == 105 || meno[strlen(meno)-1] == 117 || meno[strlen(meno)-1] == 121 || meno[strlen(meno)-1] == 99)
	    {
	    strcat(meno, "i");
	    return meno;
	    }
	//ak nje tak to bude dajme tomu zena...
	else 
		{
		strcat(meno, "e");
		return meno;
...
char *
akuzativ_z(char meno[USER_NAME_LEN+5])
	{
	
	//tentoraz je eqal aj zena aj ulica....
	if (meno[strlen(meno)-1] == 97)
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
	    strncpy(pomocna,meno,(strlen(meno)-1));
...
char *
lokal_z(char meno[USER_NAME_LEN+5])
	{
	
	//eqal, len zmena koncoviek....
	if (meno[strlen(meno)-1] == 97)
	    {
	    char pomocna[USER_NAME_LEN+8] = "\0";
	    strncpy(pomocna,meno,(strlen(meno)-1));
...
char *
instrumental_z(char meno[USER_NAME_LEN+5])
	{
